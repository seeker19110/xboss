...
const SHEETS = [
  {
    code: "OGTĐ",
    name: "Ống gió trục đứng",
    responsible: "Mr. Thừa",
    prefix: "A",
    floors: 8,
    subs: 6,
    dims: ["1300x700 X3-X4", "1000x600 X5-X6", "800x500 Y2-Y3"],
  },
  {
    code: "OGHL",
    name: "Ống gió hành lang",
    responsible: "Mr. Thừa",
    prefix: "H",
    floors: 8,
    subs: 5,
    dims: ["600x300", "500x250", "400x200"],
  },
  {
    code: "OGCH",
    name: "Ống gió căn hộ",
  ...
  },
  {
    ...
    floors: 6,
    subs: 4,
    dims: ["CH 23", "CH 28", "CH 33", "CH 38"],
  },
];

const SUB_NAMES = [
  "Lắp đặt support / ty treo",
  "Nẹp TDC + gioăng nối ống",
  "Lắp đặt ống gió / ống đồng",
...
async function main() {
  ...
  const projectId = await insertId(
    `INSERT INTO projects (name, code, investor, contractor) VALUES (?, ?, ?, ?)`,
    "TT AVIO Tháp A",
    "AVIO-A",
    "AVIO",
    "MEP Co.",
  ...
  for (const s of SHEETS) {
    ...
    for (let f = 1; f <= s.floors; f++) {
      ...
      for (let k = 1; k <= s.subs; k++) {
        ...
        const taskId = await insertId(
          `INSERT INTO tasks (package_id, code, seq_no, name, status, start_date, end_date, duration_days, progress_percent)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          wpId,
          `${wpCode},${String(k).padStart(2, "0")}`,
          `${f}.${String(k).padStart(2, "0")}`,
          pick(SUB_NAMES, k - 1),
          status,
          isoDays(-40 + f * 6 + k * 2 - 5),
          end,
          5,
          progress,
        );
      ...
      await run(
        ...
        wpProgress,
        deriveStatus(wpProgress, isoDays(-60 + f * 5 + 20)),
        wpId,
      );
    }
  }
  ...
  console.log(
    `✅ ${SHEETS.length} sheet types, ${totalWp} work packages, ${totalTask} tasks, ${totalDim} dimensions.`,
  );
  process.exit(0);
}
