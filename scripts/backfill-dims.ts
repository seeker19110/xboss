...
async function main() {
  ...
  for (const sheet of sheets) {
    ...
    if (labels.length === 0) {
      console.warn(
        `⚠️ Sheet ${sheet.code}: chưa có dimension nào trong DB — cần re-import từ Excel.`,
      );
      continue;
    }
    ...
    const missing = await query<Task>(
      `SELECT t.id FROM tasks t
         JOIN work_packages wp ON t.package_id = wp.id
        WHERE wp.sheet_type_id = ?
          AND NOT EXISTS (SELECT 1 FROM progress_dimensions pd WHERE pd.task_id = t.id)`,
      sheet.id,
    );
    ...
    for (const t of missing) {
      for (const l of labels) {
        await run(
          `INSERT INTO progress_dimensions (task_id, dimension_label, installed, value) VALUES (?, ?, 0, 0)`,
          t.id,
          l.label,
        );
        totalCells++;
