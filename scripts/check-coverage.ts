...
function parseSummary(out: string): { files: number; lines: number; branches: number; funcs: number } | null {
...
console.log(`Mốc đo ngày ${baseline.measuredAt}: files=${baseline.files} lines=${baseline.lines}% branches=${baseline.branches}% funcs=${baseline.funcs}%`);
console.log(`Đo lần này:                files=${measured.files} lines=${measured.lines}% branches=${measured.branches}% funcs=${measured.funcs}%`);
