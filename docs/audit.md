...
- **Audit "nâng cấp chuyên nghiệp hoá"** (khác audit bug-hunting ở trên — người dùng hỏi kiểu "rà toàn bộ code xem cần nâng cấp gì để chuyên nghiệp/vận hành trơn tru hơn"): vẫn chia 3 miền song song như audit toàn dự án, nhưng mỗi agent trả về **2 phần tách bạch**: (1) phát hiện lỗi/rủi ro THẬT (đúng khung §3-§7, có file:line), (2) **"Đề xuất nâng cấp chiến lược"** — cải tiến không phải bug (vd session revocation, CSRF defense-in-depth, error boundary theo segment, coverage gate CI, health-check+rollback deploy, CHANGELOG tự sinh) kèm ước lượng chi phí/tác động. Mẫu chia miền đã dùng hiệu quả (2026-07-19): (A) Bảo mật & Phân quyền + Logic nghiệp vụ (gộp — cùng thuộc "ranh giới API" và `lib/tien-do/recompute.ts`/`lib/bao-mat/auth.ts`), (B) UI/UX & A11y + Vận hành/Offline/Xuất bản (gộp — cùng trải nghiệm người dùng cuối), (C) Hiệu năng/Dependency/CI/CD + tiêu chuẩn code (tech stack, coverage, logging, release process). Tổng hợp 3 báo cáo thành 1 danh sách ưu tiên 🔴 (rủi ro thật, chi phí thấp) / 🟡 (cải thiện chuyên nghiệp hoá, đợt tới) / 🟢 (dài hạn, cần quyết định thêm — vd đổi thư viện data-fetching không được tự chọn thay người dùng, phải hỏi hoặc ghi rõ "chưa đủ đặc tả" thay vì tự chế).
...
- [ ] **Độ phủ test (định lượng)**: đo bằng coverage built-in của `node:test` (Node 22): `npm run test:coverage` (hoặc `tsx --test --experimental-test-coverage tests/<file>` cho 1 file). Chỉ soi **logic thuần** (`lib/**`, `app/api/**`), không đo component UI. Cơ chế **ratchet — không tệ hơn lần đo trước**: mốc hiện tại là **nguồn sự thật duy nhất** trong `coverage-baseline.json` ở gốc repo (cổng `npm run check:coverage` đọc thẳng file này) — không chép số vào tài liệu khác để khỏi lệch; thêm test mới thì nâng dần, không để trôi xuống.
...
- [ ] **Xuất PDF/Excel** (`@react-pdf/renderer`, `exceljs`): font hỗ trợ đủ dấu tiếng Việt (đã từng vỡ dấu do dùng Helvetica mặc định — dùng `lib/nen/pdf-fonts.ts` cho mọi route PDF mới); cột SQL tham chiếu đúng tên thật (đã từng có route 500 vì tham chiếu cột không tồn tại như `work_package_id`/`deadline` — chạy thử route thật, không chỉ đọc query bằng mắt).
