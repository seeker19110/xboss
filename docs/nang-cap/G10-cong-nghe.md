...
- **Kiểm tra trạng thái hoạt động (health check):** `lib/healthcheck.ts::runHealthChecks()` — kiểm 9 hạng mục dùng API (Postgres `SELECT 1`, Telegram Bot API `getMe`) và không dùng API (XBOSS_SECRET/CRON_SECRET/SMTP/VAPID/Google Sheet đã cấu hình chưa, `data/uploads` ghi được + dung lượng, số dòng `login_rate_limits` bất thường). `GET /api/tech/health-check` (chỉ Admin, nút "Kiểm tra ngay" trên panel Hệ thống) chạy thủ công; `GET /api/cron/health-check` (Bearer `CRON_SECRET` hoặc session Admin/PM, khoá `sync_locks` chống chạy chồng) chạy 2 lần/ngày qua cron ngoài (xem `DEPLOY.md` — vượt giới hạn Vercel Hobby 1 lần/ngày) — **chỉ gửi email + Telegram cho Admin khi có lỗi/cảnh báo**, chạy sạch thì im lặng. Mỗi lần chạy ghi 1 dòng vào bảng `health_check_runs` (migration 0131) làm lịch sử.
