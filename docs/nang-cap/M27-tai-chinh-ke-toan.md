...
- **Ngưỡng "tạm ứng quá hạn hoàn ứng" (PR3, chốt khi code):** schema PR1 (`migrations/0037_finance.sql`) không có cột `due_date` riêng cho `advances`, chỉ có `advance_date`. Quyết định: coi tạm ứng quá hạn khi `advance_date` đã qua **30 ngày** (`ADVANCE_OVERDUE_DAYS`, `lib/finance.ts`) mà `status <> 'settled'` — không thêm cột `due_date` mới vào schema đã chốt PR1. Notification `advance_overdue` dùng `migrations/0039_finance_notifications.sql` (cột `notifications.advance_id` + unique index dedup một phần, pattern giống `warranty_item_id`/`warranty_claim_id`).
- **Gợi ý lương từ chấm công (PR3):** `payrollFromAttendance(period, projectId?)` chỉ gộp chấm công theo NGƯỜI (`personnel_id NOT NULL`, tái dùng `attendanceSummary` ở `lib/hr.ts`); chấm công theo tổ (`headcount` gộp, `personnel_id NULL`) không tách được người cụ thể nên không đưa vào gợi ý — nhập tay các trường hợp này. Hàm chỉ trả gợi ý, không tự ghi vào bảng `payroll`.
