...
| Spec owner       | (chờ gán)                                                                                                                                                                                                                                                                                                                                                                       |
| State            | **Approved — ĐÃ THI HÀNH XONG toàn bộ 7 PR** (PR1 rule pack v4 · PR2 thư viện block · PR3 nền/tuyến/nhãn · PR4 phụ kiện/thiết bị/thư viện · PR6 trang in/mặt cắt · PR7 giá đỡ/lỗ chờ/tag/thống kê · PR5 `XBOSS_VE_DOI` + báo cáo phiên vẽ + tài liệu + rule pack v7 — 2026-08-25). Phần Adapter còn **chờ verify tay trên máy có AutoCAD 2026** (không có runner Windows — §18) |
| Người/ngày duyệt | (chờ)                                                                                                                                                                                                                                                                                                                                                                           |
| Cập nhật         | 2026-08-25 — bản đầu + bổ sung cùng ngày: trang in/mặt cắt (giữa chừng) + 5 tính năng rà sót (giá đỡ, lỗ chờ, tag, bảng thống kê, độ dốc) + mục "Phiên bản sau" §20; đóng đợt ở PR5 (sửa AC12 cho đúng số học, rule pack v7 khai item đếm giá đỡ/sleeve + khóa phụ kiện nặng)                                                                                                   |
...
> **Cập nhật 2026-08-28 — cả 6 mục đã có lối ra:** 5 mục đầu đã được viết thành đặc tả riêng
> (`M109`–`M113`, State **Draft**, chờ duyệt); mục cuối (đối chiếu chéo M101) **đã tự đóng** khi M101/M102
> triển khai. Cột "Lý do để lại" giữ nguyên làm bối cảnh — các đặc tả mới xử lý đúng những rủi ro ghi ở đó.

| Tính năng                                                                                                                    | Giá trị                                                                                                     | Lý do để lại                                                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `XBOSS_VE_NGATNET` — ngắt nét giao chéo → **`M109-ngat-net-giao-cheo.md`** (Draft)                                           | Quy ước trình bày 2D "ống dưới ngắt nét" — bản vẽ nộp đẹp chuẩn                                             | Đụng hình học hiển thị quanh tim — phải thiết kế để gap CHỈ ở nét biên/hiển thị, tim giữ nguyên (không được lệch BOCKL); cần nghĩ kỹ                           |
| `XBOSS_VE_REV` — revision cloud + tam giác → **`M110-revision-cloud.md`** (Draft, chỉ phần CAD)                              | Đánh dấu vùng sửa giữa các revision, liên kết `drawing_revisions` trên server — truy vết CĐT yêu cầu sửa gì | Nên đi cùng chu trình duyệt bản vẽ trên web (trạng thái revision, so sánh 2 bản) — một đợt riêng trọn vẹn hơn                                                  |
| Nhân bản tầng điển hình → **`M111-nhan-ban-tang-dien-hinh.md`** (Draft)                                                      | Copy cả hệ sang N tầng (tháp căn hộ AVIO tầng điển hình nhiều), giữ XData, tự đổi tag `{floor}` + tên vùng  | Giá trị lớn nhưng rủi ro nhân bản lỗi hàng loạt — chờ M100 lõi chạy ổn qua pilot                                                                               |
| Sơ đồ đứng (riser) bán tự động → **`M112-so-do-dung-riser.md`** (Draft, sau M111)                                            | Dựng riser từ dữ liệu tuyến các tầng                                                                        | Cần dữ liệu liên tầng có cấu trúc (bản vẽ nào = tầng nào, điểm trục đứng) — gần bài toán BIM; chỉ khả thi sau khi nhân bản tầng + vùng/tầng của M101 chạy thật |
| Thư viện block theo dự án → **`M113-thu-vien-block-theo-du-an.md`** (Draft)                                                  | Mỗi dự án/CĐT một bộ block riêng                                                                            | Đã chốt bản đầu toàn cục; xem lại sau UAT                                                                                                                      |
| Đối chiếu chéo M101 — ✅ **ĐÃ ĐÓNG** (phép kiểm 17 ở M102; item `support-hanger`/`sleeve-opening` trong takeoff từ M100 PR5) | Tag trùng vào `XBOSS_KIEMTRA` (phép 17), giá đỡ/sleeve vào bóc theo vùng                                    | Tự động có khi M101 triển khai — ghi để không quên nối 2 đặc tả                                                                                                |
