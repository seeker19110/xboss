...
| `M51-da-du-an-rls.md`           | RLS theo dự án + `organizations` (GĐ0 của M54)                                                  | ⚠️ GĐ0 xong, chờ merge/deploy                                   | Nợ: khoá cửa PR2 sau ~1 tuần theo dõi production                                                                                                                                     |
...
| `M58-qr-offline-hien-truong.md` | QR tem in `/r/<kind>/<id>` + offline queue IndexedDB ảnh/nhật ký                                | ⚠️ PR1 + PR2 xong; **PR3 (wire ảnh/nhật ký) chưa**              | PR1: resolve `/r/[kind]/[id]` + tem in. PR2: `app/components/offlineQueue/` (logic/store/image + hook + badge AppHeader), di trú êm từ localStorage, quota ảnh 50MB, Background Sync |
