...
> **M105 — Tự động phân chia đốt toàn hệ MEPF theo kiểu kết nối** (`M105-chia-dot-mepf-theo-kieu-noi.md`): 📝 **Draft chờ duyệt (2026-08-26)** — chia đốt MỌI tuyến MEPF vẽ bằng `XBOSS_VE` (ống gió nẹp C max 1180 / TDC max 1110 / mặt bích V max 1180 — số người dùng chốt; ống nước/PCCC theo cây thương phẩm ren/grooved/hàn/măng xông; máng cáp thanh + tấm nối) bằng 1 engine tổng quát tham số hóa qua rule pack v9 `jointRules`; engine C#+TS dùng chung test vector, lệnh `XBOSS_VE_CHIADOT`, migration `0143`, API + bảng đốt trên web, QTO phụ kiện mối nối. Các giá trị ⚠GIẢ ĐỊNH (dải chọn kiểu nối, khe, đốt tối thiểu, cây pipe/tray, divideMode, định mức hardware) chờ người dùng chốt ở §13 trước khi Approved.
