...
7. **M54 GĐ1** — multi-tenant SaaS (phụ thuộc cứng M51). **PR1 (trục `org_id`) đã xong** 2026-07-21 (nhánh `claude/feat-m54-gd1-pr1-org-axis`, `migrations/0078_org_axis.sql`, xem `PROGRESS.md`); PR2+ (session mang orgId, RLS org, object storage) còn lại.
...
| `M54-multi-tenant-saas.md`      | Trục `org_id` + RLS org + object storage uploads (GĐ1)                                          | ⚠️ PR1 xong (org_id axis)                | PR1 `migrations/0078_org_axis.sql` (trục org_id + boq_codes PK `(org_id,code)`) xong 2026-07-21; PR2+ (session orgId, RLS org, storage) còn lại. Phụ thuộc cứng M51                                                                                                                                                                                                                                                                                            |
