...
| `M58-qr-offline-hien-truong.md` | QR tem in `/r/<kind>/<id>` + offline queue IndexedDB ảnh/nhật ký                                | ⚠️ PR2 xong (khung offline IndexedDB); **PR1 (QR) + PR3 (wire ảnh/nhật ký) chưa** | PR2: `app/components/offlineQueue/` (logic/store/image + hook + badge AppHeader), di trú êm từ localStorage, quota ảnh 50MB, Background Sync |
