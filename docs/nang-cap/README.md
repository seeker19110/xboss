...
> **M100 tiến độ thi hành:** PR2 (thư viện block: `migrations/0139_cad_block_libs.sql`, `lib/ky-thuat/cad/block-lib.ts`, `app/api/engineering/cad/block-lib/`, mục "Thư Viện Block" trên `/engineering/chuan-hoa-ban-ve`, `XBoss.Cad.Core/Draw/BlockManifest.cs`) **đã làm 2026-08-25** — xem `PROGRESS.md`. PR1/PR3–PR7 chưa.
...
> **M101 tiến độ thi hành:** PR1 (rule pack `v5.json` + 7 phép kiểm mới trong `XBoss.Cad.Core/Inspection/PhepKiemMoRong.cs` + `Geometry/Segment2D.cs`, mọi phép mặc định TẮT, Adapter chưa đụng) **đã làm 2026-08-25** — xem `PROGRESS.md`. PR2–PR5 chưa.
