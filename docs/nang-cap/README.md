...
>
> **Chưa triển khai (theo thứ tự thi hành đã chốt):** M54 GĐ1 — xem bảng "Đặc tả chờ triển khai" bên dưới (M53 toàn bộ 4 PR, M55 + M57 PR1+PR2 + M61 + M51 GĐ0 + M56 toàn bộ 2 PR đã xong; **M58 PR3 (PR #284) + M59 PR1 (PR #285) đã code xong, đang chờ CI/merge**). Một số việc **hoãn có chủ đích** (M49 PR3 SSO OIDC merge nhưng flag tắt, M60 major deps) — xem `PROGRESS.md` mục "Việc tạm hoãn".
>
...
6. ~~**M58 PR3** — wire ảnh/nhật ký vào khung offline queue~~ → **đã code xong** (2026-07-19, nhánh `claude/feat-m58-pr3-wire-offline`, PR #284 — rebase lên `main` mới nhất + lint/typecheck/build/test xanh cục bộ, chờ CI/merge).
7. **M54 GĐ1** — multi-tenant SaaS (phụ thuộc cứng M51). Việc kế tiếp.
...
| `M55-bi-metabase.md`            | Schema `bi` (view whitelist cột) + role `xboss_bi` chỉ-đọc cho Metabase                         | ⚠️ PR #270 mở (chưa merge)                         | 18 view + role riêng chỉ-đọc, test bất biến cột-cấm, vận hành `docs/ops/metabase.md`, migration `0073`                                                                                                                                                                                                                                                                                                                                                         |
...
| `M58-qr-offline-hien-truong.md` | QR tem in `/r/<kind>/<id>` + offline queue IndexedDB ảnh/nhật ký                                | ✅ cả 3 PR xong (PR3: PR #284 chờ merge) | PR1: resolve `/r/[kind]/[id]` + tem in. PR2: `app/components/offlineQueue/` (logic/store/image + hook + badge AppHeader), di trú êm từ localStorage, quota ảnh 50MB, Background Sync. PR3 (2026-07-19, PR #284): dedup ảnh qua `sha256` (`migrations/0075_task_photos_hash.sql`), `PhotosModal`/`DiaryEditorModal` wire vào offline queue (`enqueuePhoto`, nháp `diary_note` full-replace)                                                                                                                                                               |
