...
> **M104 — Thêm block trực tiếp từ web** (`M104-them-block-truc-tiep-tu-web.md`): server + web **đã làm 2026-08-25**, route `POST /api/engineering/cad/block-lib/blocks` + `GET ?file=` (tệp DWG lẻ lưu riêng), form kéo-thả, advisory lock chống đua, 15 ca test; plugin đọc đa tệp **đang làm nốt cùng ngày**.
