...
>
> **Chưa triển khai (theo thứ tự thi hành đã chốt):** M53 PR4 → M55 → M58 PR3 → M54 GĐ1 → M59 — xem bảng "Đặc tả chờ triển khai" bên dưới (M53 PR1-3, M57 PR1, M51 GĐ0, M56 PR2 đã xong). Một số việc **hoãn có chủ đích** (M49 PR3 SSO OIDC merge nhưng flag tắt, M60 major deps) — xem `PROGRESS.md` mục "Việc tạm hoãn".
>
...
1. ~~**M53 (4 PR) song song M57 PR1**~~ → **PR1-3 của M53 + PR1 của M57 đã xong** (2026-07-18, merge `main` qua PR #252, commit `cefda6a`). Chỉ còn **M53 PR4** (audit state in-process khi chạy cluster + tài liệu vận hành).
2. **M53 PR4** — audit state in-process + tài liệu vận hành (nhỏ, việc kế tiếp).
3. ~~**M56 PR2** — bắt buộc 2FA theo vai trò~~ → **đã xong** (2026-07-18, nhánh `claude/feat-m56-pr2-bat-buoc-2fa`, KHÔNG migration — dùng `code_lists`; chặn ở `proxy.ts` Node Middleware, cờ `mustSetup2fa` trong token 5 phần).
4. ~~**M61** — override quyền theo dự án~~ → **đã xong** (2026-07-18, PR1 #248 + PR2 #249, đã merge vào `main`).
5. ~~**M51 (GĐ0 của M54)** — RLS theo dự án + `organizations`~~ → **GĐ0 xong** (2026-07-18, PR #256, đã merge vào `main`; nợ bước "khoá cửa" PR2, xem `PROGRESS.md`).
6. **M55** — BI/Metabase (cần dữ liệu ổn định sau RLS để view whitelist đúng).
7. **M58 PR3** — wire ảnh/nhật ký vào khung offline queue (PR1+PR2 đã xong).
8. **M54 GĐ1** — multi-tenant SaaS (phụ thuộc cứng M51).
9. **M59** — histogram tài nguyên (không migration, chỉ tổng hợp — làm cuối, mọi bảng nguồn đã chốt).
...
| ------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `M53-scale-headroom.md`         | Đo tải → watermark SSE O(1) thay aggregate JOIN 3s/client, pool env                             | ⚠️ PR1-3 xong (`0067_sheet_versions.sql`, pool env `lib/db/index.ts`, quan trắc `app/api/health`); **PR4 (audit state in-process + docs vận hành) chưa** | Merge #252 (`cefda6a`, 2026-07-18); việc kế tiếp trong hàng đợi                                                                                                                                                                                                                                  |
| `M56-2fa-totp.md`               | TOTP RFC 6238 + recovery codes; PR2 bắt buộc theo vai trò                                       | ✅ xong (PR1 `0065_totp.sql`; PR2 enforce theo role — KHÔNG migration)                                                                                   | PR2: cờ `mustSetup2fa` trong token phiên 5 phần, chặn ở `proxy.ts` (Node Middleware) 403 mọi API trừ `/api/auth/*`; domain `require_2fa_roles` trong `code_lists`                                                                                                                                |
...
| `M57-tim-kiem-toan-van.md`      | FTS GIN index + `unaccent` (thay ILIKE inline hiện tại)                                         | ✅ PR1 xong                                                                                                                                              | Merge #252 (`cefda6a`, 2026-07-18) — `lib/search.ts`, `migrations/0068_fts.sql`; PR2 (extract text PDF) tuỳ chọn, không nằm trong hàng đợi                                                                                                                                                       |
