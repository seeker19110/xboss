...
>
> **Chưa triển khai (theo thứ tự thi hành đã chốt):** M58 PR3 → M54 GĐ1 → M59 — xem bảng "Đặc tả chờ triển khai" bên dưới (M53 toàn bộ 4 PR, M55 + M57 PR1+PR2 + M61 + M51 GĐ0 + M56 toàn bộ 2 PR đã xong). Một số việc **hoãn có chủ đích** (M49 PR3 SSO OIDC merge nhưng flag tắt, M60 major deps) — xem `PROGRESS.md` mục "Việc tạm hoãn".
>
...
8. **M59** — histogram tài nguyên (không migration, chỉ tổng hợp). **PR1 xong** (2026-07-18, nhánh `claude/feat-m59-pr1-resources`); PR2 (cảnh báo `resource_conflict` + cột quá tải `/lookahead`) chưa.
...
| `M59-tai-nguyen.md`             | Histogram nhân lực/thiết bị kế hoạch-vs-thực-tế, cảnh báo gán chồng                             | ⚠️ PR1 xong; **PR2 (cảnh báo + lookahead) chưa**   | PR1 (2026-07-18, nhánh `claude/feat-m59-pr1-resources`, KHÔNG migration): `lib/resources.ts` + `GET /api/resources` + trang `/resources`. **BỎ xung đột thiết bị** (equipment_logs không có dải ngày). Equipment KHÔNG scope dự án (bảng `equipment` không có `project_id`). PR2: notification `resource_conflict` + cột quá tải trong `/lookahead`                                                                                                            |
