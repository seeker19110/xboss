...
> **M102 tiến độ thi hành:** PR1 (rule pack `v8.json` + validator 2 tầng + phép kiểm 17/18 trong `XBoss.Cad.Core/Inspection/PhepKiemMoRong.cs` + Core bước 12/13 trong `Standardize/ChuanHoaMoRong.cs` + test xunit/node:test + sửa `knownIssues`) **đã làm 2026-08-25**; PR2 (Adapter thi hành bước 12/13 trong `StandardizePipeline.cs` + quét tag cho phép kiểm 17) chưa.
