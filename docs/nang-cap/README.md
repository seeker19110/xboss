...
> **M105 — Tự động phân chia đốt ống gió theo kiểu kết nối** (`M105-chia-dot-ong-gio-theo-kieu-noi.md`): 📝 **Draft chờ duyệt (2026-08-26)** — chia đốt tuyến ống gió vẽ bằng `XBOSS_VE` theo kiểu nối (nẹp C max 1180 / TDC max 1110 / mặt bích V max 1180 — số người dùng chốt), engine hình học thuần C#+TS dùng chung test vector, lệnh `XBOSS_VE_CHIADOT`, migration `0143`, API + bảng đốt trên web, QTO phụ kiện mối nối theo rule pack. Các giá trị ⚠GIẢ ĐỊNH (dải chọn kiểu nối theo cạnh lớn, khe gioăng, đốt tối thiểu, định mức phụ kiện) chờ người dùng chốt ở §13 trước khi Approved.
