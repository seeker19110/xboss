...
> ✅ Đã duyệt — được code. 4 open decision ở §18 (O1–O4) **đã chốt hết khi thi hành PR2**.
...
| **O1** ✅ | **Rasterize SVG→PNG trên server**: chọn thư viện nào, hay gửi thẳng SVG? | **CHỐT ở PR2: gửi thẳng nguồn SVG, không rasterize, không thêm phụ thuộc.** `dungPreviewSvg` sinh SVG thuần từ hình học DXF (line/arc/circle) — với block CAD, _nguồn SVG chính là hình học_, không phải ảnh chụp lại của hình học; rasterize chỉ mất độ chính xác và thêm một thư viện render nặng. Tầng 3 vì vậy đổi tên từ "vision" thành **"hình học"** (`nguon_quyet_dinh = 'hinh_anh'` giữ nguyên để khỏi phải đổi migration) |
| **O2** ✅ | Ngưỡng độ tin cậy để một dòng được **chọn sẵn** trong bảng duyệt         | **CHỐT 0.80** — hằng `NGUONG_CHON_SAN` trong `lib/dich-vu/cad-block-phan-loai.ts`; route chi tiết lô trả kèm để UI dùng đúng một nguồn. Đo lại trên bộ đối chứng sau khi có nhãn xác nhận rồi điều chỉnh                                                                                                                                                                                                                            |
| **O3** ✅ | Có gửi tên dự án/tên tệp kèm theo prompt để tăng độ chính xác không?     | **CHỐT KHÔNG.** Dữ liệu gửi ra ngoài giới hạn ở **từ vựng rule pack** (id hạng mục, id hệ, tên block đã có trong thư viện) + tên/layer/thuộc tính/hình học của chính block đang xét. Không tên dự án, không dữ liệu tài chính/nhân sự, không tệp bản vẽ                                                                                                                                                                             |
| **O4** ✅ | (phát sinh khi thi hành PR2) Có cần Batches API không?                   | **KHÔNG.** §4 dự tính một lượt gọi cho mỗi block nên mới cần batch giảm 50%. Thi hành thật thì tầng 2 là **một lượt gọi cho CẢ LÔ** (danh sách tên rất nhẹ) và tầng 3 chia mẻ 25 block — số lượt gọi tỉ lệ với số _mẻ_, không phải số block. Batches API chỉ thêm hạ tầng polling mà không giảm được gì đáng kể                                                                                                                     |
