...
| Spec owner       | (chờ gán)                                                                                                                                                                                                                            |
| State            | **Approved for implementation** — PR0 (đã merge) + PR1 (rule pack v1, đã merge); PR-A (rule pack v2 + khung plugin + KIEMTRA/CHUANHOA/BOCKL/Excel — nhánh `claude/autocad-csharp-plugin-ypi9nb`); PR-B (9 phép kiểm + JSON kiểm tra + SUBTOTAL Excel + sidecar JSON + `XBOSS_BATCH` — nhánh `claude/plugin-upgrade-m8z0hx`); PR2/PR5 chờ điều kiện ngoài (§18) |
