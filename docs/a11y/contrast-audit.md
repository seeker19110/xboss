...
| ✅ P2 | `app/tracking/[sheet]/page.tsx` (lưới chính)                                                                                                                            |    46 |    28 |    74 | **Xong** — axe desktop+mobile xanh; chỉ sửa node axe báo (nhãn ngày/tầng/số task/nhãn nhỏ → zinc-400; 2 select +aria-label), không đụng hover/icon                                                                                       |
...
| 🟡 P3 | `gantt` (7+3), ~~`timeline`~~ ✅, ~~`lookahead`~~+~~`LookaheadTable`~~ ✅                                                                                               |    17 |    11 |    28 | **lookahead + timeline xong** (axe desktop+mobile). timeline: ô 0% `bg-zinc-800`→`text-zinc-300`, legend/th/hint→zinc-400, select +aria-label, 2 vùng cuộn +`tabIndex`/`role`. lookahead: trang IN nền sáng (muted→zinc-600). Còn gantt. |
...
3. ~~tracking grid~~, ~~my-tasks~~, ~~payments~~, ~~materials~~ (my-tasks/payments/materials qua #48/#49), ~~lookahead~~, ~~timeline~~ → **xong** (axe desktop+mobile). **Tiếp:** gantt → admin → notifications → approvals…, mỗi trang một spec authed. Gộp **~10 nút accent FAIL** (§3) khi chạm trang chứa.
   - **Bài học trang IN nền sáng** (`/lookahead`): `bg-white`/`text-zinc-900` ĐẢO theo theme (light mode remap `white`→`#18181b`) → text mờ phải **zinc-600** (an toàn cả 2 theme), KHÔNG phải zinc-400.
   - **Bài học vùng cuộn**: bảng `overflow-x-auto` cần `tabIndex={0}`+`role="region"`+`aria-label` (rule `scrollable-region-focusable`, **serious** khi bảng tràn ngang trên mobile). Anchor chờ spec neo theo **nội dung** (vd `role=region`), KHÔNG theo tiêu đề header (truncate ~0 width trên mobile hẹp).
