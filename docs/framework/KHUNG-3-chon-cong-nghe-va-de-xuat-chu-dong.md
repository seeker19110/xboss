...
> Trả lời câu hỏi: _từ một ý tưởng, AI phải nghiên cứu và đề xuất gì — về công nghệ và về mọi mặt khác — TRƯỚC khi chốt và viết code._
...
nêu thiếu sót/rủi ro/cơ hội kèm **đề xuất cụ thể**, để người dùng quyết định. Mỗi mục: _hỏi gì → đề xuất gì_.
...
| 11  | **Giao diện / theme**          | Mặc định **nền Dark blue** + chế độ **Light** (xem `BO-SUNG-chat-luong.md` PHẦN 3). Design tokens. |
...
**Mục tiêu:** đề xuất stack tối ưu _cho ý tưởng cụ thể này_, dùng **phiên bản tối ưu nhất hiện tại**,
...
- Độ trưởng thành & ổn định: tuổi đời, có LTS, lịch sử dùng trong production, tần suất breaking change _thấp_.
...
- Mức phù hợp bài toán: có giải quyết _đúng_ nhu cầu này không (realtime, SEO, edge, AI...).
...
- **Chỉ cách tân có chủ đích ở 1 chỗ mỗi lúc.** Nếu một thư viện mới mang lợi thế lớn cho _đúng_ bài
...
- **Khớp với đội ngũ:** công nghệ tốt nhất _trên giấy_ nhưng cả đội không biết → có thể chậm hơn cái "đủ tốt mà quen".
...
3. Chọn phương án thắng → **ghi ADR** (`docs/adr/`) nêu rõ _tại sao_ và _đã loại gì, vì sao_.
...
> **Không dùng máy móc:** với mỗi ý tưởng, AI phải đối chiếu PHẦN B và _biện minh lại_ (hoặc đề xuất khác).
...
> **Lưu ý quan trọng:** đây là các _major_ tương đối mới (Next 16, React 19, Tailwind 4, TS 6, Zod 4).
...
1. **Bản đề xuất công nghệ** (đưa vào `PROJECT.md` mục 4): mỗi lựa chọn + phiên bản + _ngày xác minh_ + 1 câu lý do.
