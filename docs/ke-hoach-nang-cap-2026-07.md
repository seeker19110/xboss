...
4. **Nợ a11y contrast còn lại:** ~~`/notifications`, `/admin`~~ **đã xong** (2 trang mật độ cao nhất — `e2e/authed/admin.spec.ts` + `notifications.spec.ts`, desktop+mobile, axe xanh; kèm 2 fix a11y ngoài contrast: `aria-label` cho `UserSelect`, `role="switch"` cho toggle `PrefRow`). Còn lại: `/timeline`, `/gantt`, `/materials/reports`, `/materials/import`, `/materials/purchase-orders`, `/lookahead`, `/report`, `/import` — theo đúng quy trình đã dùng: thêm `e2e/authed/<trang>.spec.ts`, chạy axe, sửa, xác nhận xanh.
