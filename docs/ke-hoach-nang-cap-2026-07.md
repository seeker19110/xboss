...
4. **Nợ a11y contrast còn lại:** ~~`/notifications`, `/admin`~~ → ~~`/approvals`, `/users`, tab Yêu cầu mua trong `/materials`~~ **đã xong** (đợt 2026-07-05: `e2e/authed/approvals.spec.ts` + `users.spec.ts` + mở rộng `materials.spec.ts`, desktop+mobile, axe xanh 45/45 — fix contrast zinc-500/600→400, 2 select thiếu tên ở `/users`, link `/password` thiếu gạch chân, nút amber-600/emerald-600 chữ trắng →-700, 2 vùng cuộn bảng `/approvals` thêm tabIndex/role, 2 nút X icon-only thêm `aria-label`). Các trang còn lại trong backlog cũ (`/timeline`, `/gantt`, `/materials/reports`, `/materials/import`, `/materials/purchase-orders`, `/lookahead`, `/report`, `/import`) làm dần cùng module M0 (AppShell chạm mọi trang) — không còn là gate.
5. ~~**Siết Lighthouse a11y `warn` → `error`**~~ → **đã siết** (`lighthouserc.json`, đợt 2026-07-05): `categories:accessibility` = `error` minScore 0.9; performance/best-practices/seo giữ `warn`.
