...
`npm run cad:mau-ban-ve -- --kiem` báo đỏ nếu tệp lệch script.

> **Vì sao script không tự viết DXF thẳng ra:** bản đầu (2026-08-25) viết DXF tối thiểu đủ cho
> parser của XBoss, và **AutoCAD từ chối mở** — thiếu bảng LTYPE `ByBlock`/`ByLayer`, STYLE,
> VPORT, handle. Đúng lớp sự cố "hợp lệ theo parser ≠ AutoCAD mở được" đã ghi trong `PROGRESS.md`.
> Nay script dựng hình học thô rồi ghi lại bằng chính `exportDxf` của repo (bộ ghi đã tôi luyện
> qua nhiều vòng đối chiếu AutoCAD thật). Hệ quả: tệp có thêm layer `0` mặc định (5 layer). Cùng một hình học, khác đơn vị vẽ:
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| 4 layer sai chuẩn (+ layer `0` mặc định) (`01_M_ONG_GIO_CAP_CHINH`, `03_P_ONG_NUOC_LANH_CHW`, `08_G_GHI_CHU_DIM_TEXT`) + 1 layer không thuộc hệ nào (`ZZZ_KHONG_KHOP_GI`, phải **giữ nguyên**) | **AC1** ánh xạ layer + fallback                          |
