...
| Lệnh                   | Chức năng                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `XBOSS_VE_HANHLANG`    | **Hành lang đi ống** (M114, cần `drawTools.routingPolicy.enabled = true` — mặc định TẮT): khai một lần cho cả tầng phần dữ liệu nền của đi tuyến tự động. 4 chế độ trong một lệnh — `VEMOI` vẽ tim hành lang (chỉ đoạn thẳng), `NHAN` nhận polyline có sẵn (**không đụng một tọa độ đỉnh nào**, chỉ đổi layer + gắn XData; arc/spline/polyline có đoạn cung và đối tượng xref bị bỏ qua kèm lý do đếm được), `SUA` ghi đè bề rộng/cao độ/hệ được phép (**sổ chiếm làn giữ nguyên**), `XOA` — hành lang còn hệ đi qua thì nêu đúng hệ nào rồi hỏi lại, xóa xong các tuyến đó thành tuyến thường chứ không bị xóa theo. Bề rộng khả dụng và cao độ đáy dầm/trần **hỏi, không suy**; sổ chiếm làn sống trong XData của chính hành lang nên hệ chạy sau đọc được cả khi mở lại bản vẽ hôm khác                                                                                                                                                                                                  |
...
| `XBOSS_VE_TUYENTUDONG` | **Đi tuyến tự động theo hành lang** (M114, cần `drawTools.routingPolicy.enabled = true` — mặc định TẮT): nối cả một hệ trong **một lượt**. Quét chọn thiết bị đích (Enter = mọi thiết bị của hệ chưa có tuyến) → chọn polyline kín làm **vùng cấm** (Enter = bỏ qua) → bấm **điểm nguồn** → hộp thoại chọn hệ/loại tuyến/cỡ (+ độ dốc & cao độ nếu hệ tự chảy) kèm **bảng xem trước bắt buộc**: số thiết bị nối được, tổng chiều dài, số co, tầng/làn được cấp trong từng hành lang và **danh sách không giải được kèm lý do**. Tuyến đề xuất hiện bằng **nét mảnh tạm màu tím** — không bấm OK thì bản vẽ **không đổi một thực thể nào**. Chấp nhận → sinh polyline tim đúng chuẩn `XBOSS_VE` (+ nét biên), ghi sổ chiếm làn vào XData hành lang; 1 UNDO hoàn tác cả lượt. Chạy lại: tuyến tự động cũ được **dựng lại**, tuyến kỹ sư đã **sửa tay giữ nguyên**. Không giải được thì **nói không giải được** — lệnh không nới bán kính rẽ nhánh, không hạ độ dốc, không ép hai hệ chung làn |
...
| `XBOSS_VE_NHANTANG`    | **Nhân bản tầng điển hình (M111):** quét chọn hệ của tầng nguồn (lọc theo `drawTools.floorPolicy.copyRoles`) → xem trước **bắt buộc** (số đối tượng theo vai trò, vị trí đặt từng tầng đích, ví dụ tag trước → sau) → chép sang N tầng trong **một transaction** (1 UNDO cho cả N tầng): `DeepCloneObjects` rồi **ghi đè** XData bản chép theo bảng ánh xạ `IdMapping` (handle ngoài tập chọn bị gỡ khỏi XData, không giữ liên kết về tầng nguồn), đổi tag `{floor}`, đổi tên vùng bóc theo `zoneNamePattern` (trùng tên → dừng cả lệnh), gỡ dấu bóc của bản chép. Tầng đích đã chép rồi → hỏi _bỏ qua_ (mặc định) hay _chép đè_. Tầng nguồn đang có lỗi `XBOSS_KIEMTRA` chỉ **cảnh báo, không chặn**. `drawTools.floorPolicy.enabled=false` (mặc định) → lệnh dừng kèm hướng dẫn bật                                                                                                                                                                                                       |
...
| `XBOSS_VE_NGATNET`     | **Ngắt nét giao chéo** (M109, cần `drawTools.crossingPolicy.enabled = true` — mặc định TẮT): dò mọi chỗ hai tuyến khác hệ cắt nhau, tuyến ĐI DƯỚI bị ngắt nét theo hạng ưu tiên hệ (`priority`) — tuyến 2 nét biên dùng `Wipeout` che, tuyến đơn nét thêm cung cầu vượt `jogRadiusMm`. Đảo chiều tay từng cặp trong hộp thoại, dấu đảo ghi vào bản vẽ nên chạy lại vẫn giữ. **Tim không bị cắt hay đổi tọa độ** — chỉ sinh đối tượng hiển thị trên layer `<layer tim>XING`. Chạy lại là idempotent                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `XBOSS_VE_NGATNET_XOA` | Gỡ sạch đối tượng ngắt nét (vùng che + cầu vượt) trong vùng chọn hoặc cả bản vẽ. Không cần rule pack — đường lui luôn dùng được. Lưu ý: lệnh gỡ ĐỐI TƯỢNG, còn thứ tự vẽ mà lệnh ngắt nét đã đẩy cho tuyến đi trên thì phải `UNDO` mới về như cũ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `XBOSS_VE_REV`         | **Khoanh revision cloud** (M110): so bản vẽ với mốc của lần chốt trước rồi **đề xuất vùng cần khoanh** (thêm/xóa/đổi — vùng của đối tượng đã xóa nằm ở **vị trí cũ**), hoặc tự khoanh bằng chuột. Vẽ cloud + tam giác số revision trên layer con `<revisionPolicy.layer>-R{n}`, XData đi CẶP cloud ↔ tam giác. Chạy lại trên cùng vùng **cập nhật tại chỗ**, không nhân đôi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `XBOSS_VE_REV_CHOT`    | **Chốt revision**: hỏi ngày/nội dung/người → ghi bảng revision vào attribute khung tên của **mọi layout** theo `titleblockAttrPattern` (layout thiếu thẻ → bỏ qua kèm cảnh báo, **không** tự thêm attribute), lưu mốc so sánh mới, tăng số revision. Vượt `maxRows` → **dừng**, không bao giờ ghi đè dòng cũ; còn vùng đã đổi mà chưa khoanh → cảnh báo rồi để kỹ sư quyết                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
> `XBOSS_KIEMTRA` có thêm phép kiểm **20 — cloud/tam giác revision mồ côi** (tự tắt khi bản vẽ không có
> đối tượng revision nào, nên cloud vẽ tay bằng `REVCLOUD` không bị báo oan).
...
> **Rule pack v12+ (M111)** khai `drawTools.floorPolicy` (danh sách tầng, kiểu dời, bước cao độ,
> `zoneNamePattern`, `copyRoles`) cho lệnh `XBOSS_VE_NHANTANG`, mặc định `enabled: false`. **PR3**
> thêm phép kiểm **19** cho `XBOSS_KIEMTRA` — handle mồ côi trong bản chép tầng
> (`nhantang-handle-mo-coi`): với mọi đối tượng mang XData `TangNguon`/`NhanTang`, mọi handle nó
> tham chiếu (tim/biên/nhãn/tuyến cắt/cặp đôi/…) phải phân giải được và thuộc **đúng tầng chép đó**
> — bắt đúng lỗi guardrail của M111 §2 khi `FloorReplicator.AnhXaXData`/`DeepCloneObjects` bỏ sót
> ánh xạ. Phép này **KHÔNG có cờ `enabled` riêng** trong rule pack (cùng khuôn phép kiểm revision
> của M110): tự tắt khi bản vẽ chưa từng chạy `XBOSS_VE_NHANTANG`, nên không cần bật/tắt theo dự án.
...
| `XBOSS_TUYEN_GOIY`     | **Tuyến gợi ý từ sơ đồ nguyên lý** (M117): tải graph sơ đồ đã **chốt trên web** (API `:id/plugin`, cache offline như M113) → kỹ sư bấm điểm nguồn trên mặt bằng tầng đang mở → ánh xạ nút thiết bị của sơ đồ ↔ block đã đặt trên mặt bằng (tag → tên block) → sinh **tuyến tim NHÁP** bằng routing hành lang M114 trên layer riêng `XBOSS-GOIY`, XData hệ/cỡ điền sẵn từ graph. Thiếu ánh xạ ⇒ liệt kê, vẫn sinh phần tìm được; chưa có hành lang ⇒ dừng sạch kèm câu "chạy `XBOSS_VE_HANHLANG` trước". **Idempotent theo id graph**: chạy lại xóa đúng nháp của chính sơ đồ đó rồi dựng lại (nháp đã sửa tay thì hỏi trước) |
| `XBOSS_TUYEN_GOIY_XOA` | Dọn sạch tuyến nháp gợi ý của **một** sơ đồ (gõ mã) hoặc **mọi** sơ đồ (Enter). Chỉ đụng polyline mang dấu gợi ý — tuyến vẽ tay và tuyến của lệnh khác giữ nguyên                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
2. **Kỹ sư** vẽ **line/pline tuyến tim** bằng AutoCAD thuần, từ điểm nguồn tới từng thiết bị (block
   ...
   _Hoặc (M117):_ chạy `XBOSS_TUYEN_GOIY` để plugin sinh sẵn tuyến tim **nháp** từ sơ đồ nguyên lý
   đã chốt trên web, rồi sửa nháp như tuyến vẽ tay — từ bước 3 trở đi quy trình y hệt.
...
4. Làm shop drawing bằng bộ lệnh vẽ: `XBOSS_VE_NEN` → `XBOSS_VE` → `XBOSS_VE_PHUKIEN` /
   ...
   Vẽ xong 1 tầng điển hình → `XBOSS_VE_NHANTANG` chép sang N tầng còn lại (kèm ánh xạ lại toàn bộ
   handle + đổi tag/tên vùng) → chạy lại `XBOSS_KIEMTRA` (phép 19) để chắc bản chép không mồ côi
   handle nào, rồi mới dựng trang in/bảng thống kê riêng cho từng tầng mới.
