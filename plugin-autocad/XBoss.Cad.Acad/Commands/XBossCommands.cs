...
public sealed class XBossCommands
{
    ...
    // ===== XBOSS_LOGIN / XBOSS_LOGOUT =====

    [CommandMethod("XBOSS_LOGIN")]
    public void DangNhap()
    {
        if (SanSang() is not (var doc, var ed)) return;
        _ = doc;

        // URL server: nhớ giữa các phiên (%APPDATA%\XBoss\server.json — không nhạy cảm).
        var urlLuu = XBossApiClient.DocServerUrl() ?? "";
        var url = HoiChuoi(ed, "URL server XBoss (vd https://xboss.congty.vn)", urlLuu);
        if (string.IsNullOrWhiteSpace(url)) return;
        url = url.Trim();
        if (!url.StartsWith("http://", StringComparison.OrdinalIgnoreCase) &&
            !url.StartsWith("https://", StringComparison.OrdinalIgnoreCase))
        {
            ed.WriteMessage("\n[XBoss] URL phải bắt đầu bằng http:// hoặc https://.\n");
            return;
        }

        try
        {
            // Token đã có sẵn → thử tải rule pack luôn, chỉ ghép lại khi 401 (AC7).
            var tokenCu = CredentialStore.DocToken();
            if (tokenCu is not null)
            {
                var json = XBossApiClient.TaiRulePack(url, tokenCu);
                if (json is not null)
                {
                    var packCu = RulePackStore.ImportNoiDung(json);
                    XBossApiClient.GhiServerUrl(url);
                    ed.WriteMessage($"\n[XBoss] Thiết bị đã ghép sẵn — rule pack {packCu.Version} đã cập nhật từ server.\n");
                    return;
                }
                ed.WriteMessage("\n[XBoss] Token đã hết hạn hoặc bị thu hồi trên web — cần ghép lại thiết bị.\n");
                CredentialStore.XoaToken();
            }

            // Ghép thiết bị: xin mã → người dùng duyệt trên web → poll nhận token.
            var pair = XBossApiClient.Pair(url, Environment.MachineName);
            ed.WriteMessage($"\n[XBoss] ===== GHÉP THIẾT BỊ =====\n");
            ed.WriteMessage($"[XBoss] Mã ghép:  {pair.DeviceCode}   (sống {pair.ExpiresIn / 60} phút)\n");
            ed.WriteMessage($"[XBoss] Mở trang {url.TrimEnd('/')}/engineering/thiet-bi-plugin (đăng nhập XBoss), nhập mã trên rồi bấm \"Duyệt thiết bị\".\n");

            while (true)
            {
                var hoi = new PromptKeywordOptions("\n[XBoss] Đã duyệt xong trên web chưa?") { AllowNone = false };
                hoi.Keywords.Add("KiemTra", "KiemTra", "Đã duyệt — nhận token");
                hoi.Keywords.Add("Huy", "Huy", "Hủy ghép");
                hoi.Keywords.Default = "KiemTra";
                var traLoi = ed.GetKeywords(hoi);
                if (traLoi.Status != PromptStatus.OK || traLoi.StringResult == "Huy")
                {
                    ed.WriteMessage("\n[XBoss] Đã hủy ghép thiết bị.\n");
                    return;
                }

                var poll = XBossApiClient.Poll(url, pair.DeviceCode, pair.DeviceSecret);
                if (poll is null)
                {
                    ed.WriteMessage("\n[XBoss] Mã ghép đã hết hạn — chạy lại XBOSS_LOGIN để lấy mã mới.\n");
                    return;
                }
                if (poll.Status == "pending")
                {
                    ed.WriteMessage("\n[XBoss] Chưa thấy duyệt — vào trang web duyệt mã rồi chọn KiemTra lần nữa.\n");
                    continue;
                }
                if (poll.Status == "ready" && poll.Token is not null)
                {
                    CredentialStore.GhiToken(poll.Token); // Credential Manager — KHÔNG ghi tệp (NFR4)
                    XBossApiClient.GhiServerUrl(url);
                    var hanToken = poll.ExpiresAt is { Length: >= 10 } exp ? exp[..10] : "?";
                    ed.WriteMessage($"\n[XBoss] ✔ Đã ghép thiết bị \"{poll.Name}\" (token hạn tới {hanToken}).\n");
                    var json = XBossApiClient.TaiRulePack(url, poll.Token);
                    if (json is not null)
                    {
                        var pack = RulePackStore.ImportNoiDung(json);
                        ed.WriteMessage($"[XBoss] Rule pack {pack.Version} đã tải và cache — sẵn sàng dùng các lệnh XBOSS_*.\n");
                    }
                    return;
                }
                ed.WriteMessage("\n[XBoss] Trạng thái ghép không xác định — thử lại.\n");
            }
        }
        catch (System.Net.Http.HttpRequestException e)
        {
            ed.WriteMessage($"\n[XBoss] Lỗi mạng/server: {e.Message}\n[XBoss] Vẫn dùng được rule pack cache (nếu có) — nhưng KHÔNG tải lên được cho tới khi đăng nhập lại.\n");
        }
        ...
        {
            ed.WriteMessage($"\n[XBoss] Rule pack server trả về KHÔNG hợp lệ: {e.Message}\n");
        }
    }

    [CommandMethod("XBOSS_LOGOUT")]
    public void DangXuat()
    {
        if (SanSang() is not (var doc, var ed)) return;
        _ = doc;
        var daXoa = CredentialStore.XoaToken();
        ed.WriteMessage(daXoa
            ? "\n[XBoss] Đã xoá token thiết bị khỏi Credential Manager. Thu hồi hẳn trên web: trang /engineering/thiet-bi-plugin.\n"
            : "\n[XBoss] Máy này chưa ghép thiết bị — không có token để xoá.\n");
    }
