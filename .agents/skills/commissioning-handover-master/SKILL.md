...
1. **Bất biến Thử Nghiệm An Toàn Liên Động PCCC (PCCC Interlocking Safety Invariant):**
   - Trước khi đề nghị Cơ quan Cảnh sát PCCC kiểm tra nghiệm thu, \%$ các kịch bản liên động an toàn bắt buộc phải được thử nghiệm đạt \%$ và có biên bản xác nhận:
...
2. **Bất biến Cân Bằng Khí Động Học HVAC (HVAC TAB Tolerance Invariant):**
   - Sai lệch lưu lượng gió thực tế ({\text{actual}}$) tại từng miệng gió so với lưu lượng thiết kế ({\text{design}}$) không được vượt quá dung sai chuẩn NEBB/ASHRAE:
...
- Thử áp lực thủy tĩnh đường ống nước (Hydrostatic Pressure Test tại .5 \times P_{\text{làm việc}}$ trong 2h).
...
### 4.1. [Cẩm nang kỹ thuật] hvac-tab-and-pccc-interlocking-recipes
...
1. Đo đạc tất cả các miệng gió trên cùng một nhánh ống chính, xác định miệng gió có tỷ lệ thấp nhất {\min} = \min(R_i)$.
2. Điều chỉnh van gió (OBD - Opposed Blade Damper) tại các miệng gió có tỷ lệ cao hơn để hạ dần về mức {\min}$.
...
| 4   | Quạt tăng áp buồng thang & giếng thang máy | Tắt                    | **KHỞI ĐỘNG (Duy trì áp suất - 50\text{Pa}$)** | $\le 15$ giây        |
...
### 4.2. [Cẩm nang kỹ thuật] nd06-decree24-and-cobie-handover
...
| **Floor**     | Name, Elevation                           | owers, work_fronts              | Cao độ tầng, phân khu                    |
| **Space**     | Name, RoomTag, Area                       | work_fronts                     | Phòng kỹ thuật, căn hộ                   |
| **Type**      | Name, Category, Manufacturer, ModelNumber | materials, ngineering_objects   | Chủng loại thiết bị (Model AHU, Bơm)     |
| **Component** | Name, Space, SerialNumber, TagNumber      | ngineering_object_revisions     | Cá thể thiết bị cụ thể gắn mã QR         |
| **Document**  | Name, Category, DocumentType, FileURL     | om_documents, ask_documents     | Hướng dẫn O&M, Chứng chỉ CO/CQ           |
