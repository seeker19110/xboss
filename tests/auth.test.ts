...
test("password change invalidates old token (pwFrag check)", { skip: !HAS_TEST_DB }, async () => {
  ...
  const u1 = await queryOne<{ password_hash: string }>(`SELECT password_hash FROM users WHERE id = ?`, userId);
  ...
  const u2 = await queryOne<{ password_hash: string }>(`SELECT password_hash FROM users WHERE id = ?`, userId);
...
test("material issue: race condition không xuất quá tồn kho", { skip: !HAS_TEST_DB }, async () => {
  ...
  const towerId = await insertId(`INSERT INTO towers (project_id, name) VALUES (?, 'T')`, projectId);
  const stId = await insertId(`INSERT INTO sheet_types (tower_id, code, name) VALUES (?, 'TR', 'Test Race')`, towerId);
  const matId = await insertId(
    `INSERT INTO materials (sheet_type_id, name, qty_stock, qty_used) VALUES (?, 'Vật tư test', 5, 0)`, stId);

  // Giả lập 2 request đồng thời xuất 4 đơn vị — cộng lại = 8 > 5 tồn kho.
  // Với FOR UPDATE, chỉ 1 trong 2 thành công.
  const issue = async (qty: number) => withTransaction(async () => {
    const mat = await queryOne<{ qty_stock: number; qty_used: number }>(
      `SELECT COALESCE(qty_stock,0) AS qty_stock, qty_used FROM materials WHERE id = ? FOR UPDATE`, matId);
    if (!mat || qty > mat.qty_stock) return false;
    await run(`UPDATE materials SET qty_stock = ?, qty_used = ? WHERE id = ?`,
      mat.qty_stock - qty, mat.qty_used + qty, matId);
    return true;
  });
  ...
  const final = await queryOne<{ qty_stock: number }>(`SELECT qty_stock FROM materials WHERE id = ?`, matId);
