...
// Mã duy nhất theo lần chạy — tránh đụng UNIQUE (BOQCODE, slug...) nếu lần chạy trước
...
test(
  ...
  async () => {
    ...
    try {
      ...
      projectId = await insertId(`INSERT INTO projects (name) VALUES ('M47 EVM P1 ${RUN}')`);
      towerId = await insertId(
        `INSERT INTO towers (project_id, name) VALUES (?, 'Tháp EVM')`,
        projectId,
      );
      sheetId = await insertId(
        `INSERT INTO sheet_types (tower_id, code, name, slug) VALUES (?, 'M47-EVM-${RUN}', 'Sheet EVM', 'm47-evm-${RUN}')`,
        towerId,
      );
      wpId = await insertId(
        `INSERT INTO work_packages (sheet_type_id, code, name) VALUES (?, 'M47-WP-${RUN}', 'Nhóm EVM')`,
        sheetId,
      );
      ...
      t1 = await insertId(
        `INSERT INTO tasks (package_id, code, name, start_date, end_date, progress_percent)
         VALUES (?, 'M47-T1-${RUN}', 'Task EVM 1', ?, ?, 0.5)`,
        wpId,
        daysFromTodayISO(-10),
        daysFromTodayISO(-5),
      );
      ...
      t2Id = await insertId(
        `INSERT INTO tasks (package_id, code, name, start_date, end_date, progress_percent)
         VALUES (?, 'M47-T2-${RUN}', 'Task EVM 2', ?, ?, 0)`,
        wpId,
        daysFromTodayISO(5),
        daysFromTodayISO(10),
      );
      boqId = await insertId(
        `INSERT INTO boq_items (code, name, unit, qty_contract, unit_price) VALUES ('M47-EVM-1-${RUN}', 'Ống gió EVM', 'm2', 100, 100000)`,
      );
      await run(
      ...
      if (projectId) await run(`DELETE FROM cash_transactions WHERE project_id = ?`, projectId);
      if (blId) await run(`DELETE FROM baseline_tasks WHERE baseline_id = ?`, blId);
      if (blId) await run(`DELETE FROM baselines WHERE id = ?`, blId);
      if (sheetId) await run(`DELETE FROM payment_bills WHERE sheet_type_id = ?`, sheetId);
      if (boqId) await run(`DELETE FROM boq_task_map WHERE boq_item_id = ?`, boqId);
      if (boqId) await run(`DELETE FROM boq_items WHERE id = ?`, boqId);
      if (t1 || t2Id) await run(`DELETE FROM tasks WHERE id IN (?, ?)`, t1, t2Id);
...
test(
  ...
  async () => {
    ...
    let emptyProject = 0;
    try {
      projectId = await insertId(`INSERT INTO projects (name) VALUES ('M47 EVM P2 ${RUN}')`);
      towerId = await insertId(
        `INSERT INTO towers (project_id, name) VALUES (?, 'Tháp EVM 2')`,
        projectId,
      );
      sheetId = await insertId(
        `INSERT INTO sheet_types (tower_id, code, name, slug) VALUES (?, 'M47-EVM2-${RUN}', 'Sheet EVM 2', 'm47-evm2-${RUN}')`,
        towerId,
      );
      wpId = await insertId(
        `INSERT INTO work_packages (sheet_type_id, code, name) VALUES (?, 'M47-WP2-${RUN}', 'Nhóm EVM 2')`,
        sheetId,
      );
      await run(
        `INSERT INTO tasks (package_id, code, name, start_date, end_date, progress_percent)
         VALUES (?, 'M47-T3-${RUN}', 'Task EVM 3', ?, ?, 0.5)`,
        wpId,
        daysFromTodayISO(-10),
        daysFromTodayISO(-5),
      );
      ...
      emptyProject = await insertId(`INSERT INTO projects (name) VALUES ('M47 EVM P3 ${RUN}')`);
