...
test("PATCH /api/tasks/:id: task đúng dự án của mình (pm) → 200, đổi tên thành công", S, async () => {
...
test("POST /api/tasks/:id/approve: task đúng dự án của mình (pm) → 200, nghiệm thu", S, async () => {
...
test("DELETE /api/tasks/:id/approve: task đúng dự án của mình (admin) → 200, huỷ nghiệm thu", S, async () => {
...
test("PATCH /api/tasks/batch: chứa task dự án khác (admin) → 422", S, async () => {
  ...
  const { PATCH } = await import("@/app/api/tasks/batch/route");
  const res = await PATCH(
    jreq("/x", { updates: [{ id: taskB, patch: { name: "x" } }] }, "PATCH"),
  );
  assert.equal(res.status, 422);
});

test("PATCH /api/tasks/batch: toàn bộ task đúng dự án của mình (pm) → 200, ghi đúng", S, async () => {
...
test("POST /api/tasks/:id/copy: task nguồn thuộc dự án khác (pm) → 404, không tạo bản sao", S, async () => {
...
test("POST /api/tasks/:id/copy: task nguồn đúng dự án của mình (pm) → 201, tạo bản sao", S, async () => {
...
test(
  "GET /api/subcontractors/:id/documents: NTP thuộc tổ chức khác (pm) → 404",
  S,
  async () => {
...
test("GET+POST /api/subcontractors/:id/documents: đúng tổ chức của mình (pm) → 200/201", S, async () => {
  ...
  const { POST } = await import("@/app/api/subcontractors/[supplierId]/documents/route");
  const resPost = await POST(new NextRequest("http://localhost/x", { method: "POST", body: form }), {
    params: Promise.resolve({ supplierId: String(supplierA) }),
  });
...
test(
  "GET /api/subcontractors/:id/evaluations: NTP thuộc tổ chức khác (pm) → 404",
  S,
  async () => {
