...
// lib/ky-thuat/engineering-graph.ts với `data-quality`, GIỮ NGUYÊN.)
//
// Lưu ý đã đọc code xác nhận (ghi trong báo cáo cuối, không lặp lại ở đây):
//   - Không route nào trong các route còn lại chạm lưu trữ file (đọc lib nền `engineering-zero-
//     error-tracker.ts` xác nhận: challenge/issue-certificate/reconcile-quad/verify-photo chỉ
//     nhận JSON body, không multipart/File) — không có ca upload trong file này.
