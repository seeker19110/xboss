...
- **M111 — nền cấu hình đa dự án (viết 2026-08-27) — CHƯA duyệt. MỐC 0, đi TRƯỚC M109/M110.** Người dùng bác lập luận trước đó của tôi ("nhân bản tầng điển hình trước vì AVIO nhiều tầng điển hình") bằng một điểm đúng: **plugin dùng cho nhiều dự án**, nên nhân bản tầng chỉ là tối ưu cho _hình dạng một dự án_ (vô dụng với nhà xưởng/bệnh viện/TTTM), còn auto-routing là năng lực lãi trên mọi dự án. Lập luận cũ của tôi sai ở chỗ lấy hình dạng AVIO làm mẫu số.
  - **Nhưng hệ quả lớn hơn thứ tự làm việc:** rà code cho thấy **trần đang bị chặn bởi NỀN**. `getCurrentRulePack()` trả về **một tệp JSON tĩnh toàn cục cả công ty**; `?project=` **chỉ đè đúng `takeoff.items[].boqCode`** (M101 PR4); thư viện block toàn cục (`ORG_THU_VIEN_BLOCK = 1`; migration 0139 ghi rõ "bảng KHÔNG mang org_id/project_id"). Trong khi `layerMap`, `sheetSetup`/khung tên, danh mục cỡ, `inspectionPolicy` — mỗi CĐT một khác.
  - **Và M109/M110 như đã viết sẽ làm nặng thêm:** `elevationBands` (trần chung cư 2,8 m ≠ nhà xưởng 8 m) và `routing` (bước lưới theo quy mô mặt bằng) **bản chất per-project**; khai thành khoá toàn cục là đóng đinh khuyết tật sâu thêm một tầng, và mỗi tính năng sau lại phải mở thêm một lỗ đè riêng — sửa N lần thay vì 1 lần. **Đã sửa M109/M110**: cả hai khai khoá ở **lớp dự án** và ghi phụ thuộc M111.
  - Giải pháp: rule pack **2 lớp** — lõi công ty (giữ nguyên vai trò, append-only, có version) + lớp đè theo dự án (DB, theo **đường dẫn khoá** chứ không đè cả khối, để dự án không đứng ngoài cải tiến của lõi). **Cố ý không có lớp máy cá nhân** — đó là đường trôi chuẩn mà ADR-0006 sinh ra để dẹp. Kèm thư viện block **chung + riêng dự án**, đóng nợ đã ghi ở M100 §20. Khuôn mẫu có sẵn: `cad_takeoff_boq_map` (0140) chính là lớp dự án cho `boqCode` — M111 tổng quát hoá nó.
  - **Cô lập giữa dự án là yêu cầu HỢP ĐỒNG**, không chỉ kỹ thuật: chuẩn layer/khung tên của một CĐT lộ sang CĐT khác là vấn đề pháp lý. 3 lớp phòng như `cad_takeoff_boq_map`: quyền ở route, lọc `project_id` ở tầng app, RLS ở DB.
  - 4 PR, 2 migration thêm thuần (`0145`, `0146`). 2 open decision chặn PR1: đè theo **dự án** hay theo **CĐT** (một CĐT thường có nhiều dự án cùng chuẩn), và danh sách **khoá cấm đè**.
