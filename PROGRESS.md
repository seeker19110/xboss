...
- **M108 PR4 (plugin C# `XBOSS_VE_DEXUAT_LO`) — CHƯA LÀM, bị chặn bởi môi trường.** Không cài được .NET SDK: chính sách mạng của môi trường chặn `builds.dotnet.microsoft.com` (CONNECT trả 403), và máy không có sẵn `dotnet`. Repo đặt `TreatWarningsAsErrors=true` và CI có job `plugin (dotnet Core/Tests)` build cả 4 project + chạy bộ test .NET, nên **viết C# mà không biên dịch nổi rồi đẩy lên là gần như chắc chắn làm CI đỏ** — trái thẳng với DoD "một lần push đã kiểm chứng hơn ba lần đoán". Đường web (PR3) đã dùng được đầy đủ; đường AutoCAD còn thiếu đúng lệnh gọi này.
