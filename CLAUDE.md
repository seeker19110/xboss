...
- **Điều phối & định tuyến (opusplan · Fable 5)** (quyết định 2026-07-16, thay thế "Uỷ thác theo độ khó" 2026-07-15): phiên chính (opusplan) là **người điều phối thuần** — lập kế hoạch, thiết kế, quyết định kiến trúc, viết đặc tả, định tuyến việc và duyệt kết quả cuối; **không tự code** — mọi việc implement đều giao subagent theo bảng định tuyến. Kế hoạch xuất theo mẫu `PLAN.md` (mỗi việc gắn nhãn `route:`).
  ...
  **Bảng định tuyến** — 2 trục: độ phức tạp × độ kín của đặc tả:
  ...
  - `reviewer` (Sonnet, xem `.claude/agents/reviewer.md`) — soát diff bằng skill `code-review` sau khi worker code xong, trước khi opusplan duyệt cuối. Không nằm trong bảng route (là bước hậu kiểm, không phải người làm).
  - **Khi giao việc: viết brief đầy đủ ngữ cảnh** — đường dẫn file cụ thể, quy ước dự án liên quan, tiêu chí chấp nhận rõ ràng, và (với `complex`) ranh giới quyết định được phép. Subagent KHÔNG thấy được hội thoại trước đó trong phiên, chỉ thấy đúng những gì viết trong brief.
