...
- **Uỷ thác theo độ khó** (quyết định 2026-07-15, áp dụng từ nay): phiên chính (opusplan, Opus) tập trung lập kế hoạch, thiết kế, quyết định kiến trúc, viết đặc tả đủ chi tiết (schema DDL, API, điểm chạm code, chia PR — cùng khung với `docs/nang-cap/M<xx>-*.md`). **LUÔN đọc kỹ đặc tả liên quan** (`docs/nang-cap/M<xx>-*.md`/`G<nn>-*.md`, `PROJECT.md`/`spec.md`, hoặc tài liệu nghiên cứu liên quan khác) **trước khi giao việc** — không đoán. Rồi chọn người làm theo 3 mức:
  - **Việc phức tạp** (quyết định kiến trúc, đụng nhiều file/luồng liên quan nhau, cần hiểu sâu ngữ cảnh vừa chốt trong phiên) → **Opus (phiên chính) tự code**, không giao.
  - **Việc vừa** (viết 1 tính năng/component/hàm rõ ràng đã có đặc tả cụ thể, ít phụ thuộc ngữ cảnh phiên hiện tại, tách rời được khỏi mạch quyết định vừa chốt) → giao `coder` (Sonnet, xem `.claude/agents/coder.md`): code tính năng theo đặc tả Opus vừa viết hoặc đặc tả có sẵn, fix lỗi có cách tái hiện/thông báo cụ thể, viết/bổ sung test, script backfill/import theo mẫu, refactor phạm vi rõ ràng, verify tính năng thật, xử lý review comment cụ thể.
  - **Việc cơ học** (đổi tên hàng loạt, format, việc lặp lại theo khuôn mẫu rõ ràng, không cần quyết định) → giao `mechanical` (Haiku, xem `.claude/agents/mechanical.md`): sửa lint/typecheck theo thông báo có sẵn, đổi tên hàng loạt, CRUD/route bám mẫu có sẵn, cập nhật test theo signature đã đổi.
  ...
  - **Khi giao việc: viết brief đầy đủ ngữ cảnh** — đường dẫn file cụ thể, quy ước dự án liên quan, tiêu chí chấp nhận rõ ràng. Subagent KHÔNG thấy được hội thoại trước đó trong phiên, chỉ thấy đúng những gì viết trong brief.
