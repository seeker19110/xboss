...
- `TELEGRAM_WEBHOOK_SECRET` / `ZALO_OA_SECRET` — **hiện chưa dùng**: route webhook đi vào của bot Telegram/Zalo đã bị xoá (2026-09-21/22), không route nào gọi `lib/bao-mat/webhook-inbound.ts`. Module được giữ lại (kèm test) để dùng khi thêm lại bot hiện trường: Telegram so header `X-Telegram-Bot-Api-Secret-Token`, Zalo kiểm HMAC-SHA256 trên raw body (`X-ZEvent-Signature`); route mới phải gọi `xacThucWebhookTelegram`/`xacThucWebhookZalo` ngay đầu handler, sai/thiếu chữ ký → **401** trước khi ghi DB, thiếu biến → throw fail-fast khi webhook được gọi.
