...
export default function PurchaseRequestsTab({
  ...
  const submitPR = async () => {
    if (!addDraft.materialId) {
      setError("Chọn vật tư");
      return;
    }
    if (Number(addDraft.qtyRequested) <= 0) {
  ...
  const openCreatePO = (preselectedPRs?: PR[]) => {
    const items: POItem[] = preselectedPRs
      ? preselectedPRs.map((pr) => ({
          prId: String(pr.id),
          materialId: String(pr.materialId),
          qtyOrdered: String(pr.qtyRequested),
          unitPrice: "",
        }))
  ...
  const addPoItem = () =>
    setPoItems((p) => [...p, { prId: "", materialId: "", qtyOrdered: "", unitPrice: "" }]);
  ...
  const selectPR = (i: number, prId: string) => {
    ...
    setPoItems((p) => {
      ...
      n[i] = {
        ...n[i],
        prId,
        materialId: pr ? String(pr.materialId) : n[i].materialId,
        qtyOrdered: pr ? String(pr.qtyRequested) : n[i].qtyOrdered,
      };
      return n;
    });
  };
  ...
  const submitPO = async () => {
    ...
    if (!validItems.length) {
      setError("Cần ít nhất 1 vật tư với SL > 0");
      return;
    }
    setSaving(true);
      ...
      {showCreatePO && (
        <div className="bg-zinc-800 border border-blue-700/50 rounded-lg p-4">
          ...
          <div className="mb-3">
            ...
            {/* Header */}
            <div className="hidden sm:grid grid-cols-12 gap-2 px-2 mb-1 text-xs text-zinc-400">
            ...
            <div className="space-y-1.5">
              {poItems.map((item, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center bg-zinc-700/30 rounded px-2 py-2"
                  >
                    {/* Chọn PR */}
                    ...
                    <div className="col-span-4">
                      <select
                        ...
                        onChange={(e) => updatePoItem(i, "materialId", e.target.value)}
                        className="w-full bg-zinc-700 border border-zinc-600 rounded px-2 py-1.5 text-xs text-zinc-100 outline-none focus:border-blue-500"
                      >
                        <option value="">-- Chọn vật tư --</option>
                        {materials.map((m) => (
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_CLS[pr.status]}`}
                  >
                ...
                {pr.reviewNote && (
                  <div
                    className={`text-xs italic mt-0.5 ${pr.status === "rejected" ? "text-red-400" : "text-green-400"}`}
                  >
              ...
              <div className="flex gap-1 shrink-0 flex-wrap justify-end">
                {canApprove && pr.status === "pending" && (
                  <button
                    ...
                    className="px-2.5 py-1 rounded bg-zinc-700 hover:bg-zinc-600 text-xs"
                  >
                    Xét duyệt
                  </button>
                )}
          ...
          <div className="bg-zinc-800 border border-zinc-700 rounded-xl w-full max-w-md p-5">
            ...
            <p className="text-zinc-400 text-sm mb-3">
              ...
              <strong className="text-zinc-200">
                {reviewModal.qtyRequested} {reviewModal.unit ?? ""}
              </strong>
