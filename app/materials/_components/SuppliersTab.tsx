...
function PartyForm({
  ...
  d: SupplierDraft;
  set: (fn: (d: SupplierDraft) => SupplierDraft) => void;
}) {
  return (
    <div className="space-y-3">
      <div className="space-y-1">
        <label className="text-xs text-zinc-400 block">
          ...
          Tiêu đề / Phân loại nhà cung cấp
        </label>
        <input
          value={d.title}
          onChange={(e) => set((x) => ({ ...x, title: e.target.value }))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-4 gap-y-2">
        ...
        <div className="space-y-2 border border-sky-700/50 bg-sky-950/20 rounded p-2">
          ...
          <SField
            label="Tên công ty"
            value={d.buyerCompany}
            onChange={(v) => set((x) => ({ ...x, buyerCompany: v }))}
          />
          <SField
            label="Tên công trình"
            value={d.buyerProject}
            onChange={(v) => set((x) => ({ ...x, buyerProject: v }))}
          />
          <SField
            label="Địa chỉ giao hàng"
            value={d.buyerAddress}
            onChange={(v) => set((x) => ({ ...x, buyerAddress: v }))}
          />
          <SField
            label="Đại diện"
            value={d.buyerRep}
            onChange={(v) => set((x) => ({ ...x, buyerRep: v }))}
          />
          <SField
            label="Chức vụ"
            value={d.buyerTitle}
            onChange={(v) => set((x) => ({ ...x, buyerTitle: v }))}
          />
          <SField
            label="Phone"
            value={d.buyerPhone}
            onChange={(v) => set((x) => ({ ...x, buyerPhone: v }))}
          />
        </div>
        ...
        <div className="space-y-2 border border-amber-700/50 bg-amber-950/20 rounded p-2">
          ...
          <SField
            label="Đại diện"
            value={d.sellerRep}
            onChange={(v) => set((x) => ({ ...x, sellerRep: v }))}
          />
          <SField
            label="Phone"
            value={d.phone}
            onChange={(v) => set((x) => ({ ...x, phone: v }))}
          />
          <SField
            label="Email"
            type="email"
            value={d.email}
            onChange={(v) => set((x) => ({ ...x, email: v }))}
          />
          <SField
            label="Ghi chú"
            value={d.note}
            onChange={(v) => set((x) => ({ ...x, note: v }))}
          />
        </div>
        ...
        <div className="space-y-2 border border-green-700/50 bg-green-950/20 rounded p-2">
          ...
          <SField
            label="Tên công ty"
            value={d.receiverCompany}
            onChange={(v) => set((x) => ({ ...x, receiverCompany: v }))}
          />
          <SField
            label="Địa chỉ giao hàng"
            value={d.receiverAddress}
            onChange={(v) => set((x) => ({ ...x, receiverAddress: v }))}
          />
          <SField
            label="Đại diện"
            value={d.receiverRep}
            onChange={(v) => set((x) => ({ ...x, receiverRep: v }))}
          />
          <SField
            label="Phone"
            value={d.receiverPhone}
            onChange={(v) => set((x) => ({ ...x, receiverPhone: v }))}
          />
          <SField
            label="Nhà thầu phụ"
            value={d.receiverSubcon}
            onChange={(v) => set((x) => ({ ...x, receiverSubcon: v }))}
          />
        </div>
      </div>
      ...
      <div className="space-y-2 border border-teal-700/50 bg-teal-950/20 rounded p-2">
        ...
        <SField
          ...
          onChange={(v) => set((x) => ({ ...x, deliveryTime: v }))}
        />
        <div className="grid grid-cols-2 gap-2">
          <SField
            label="Người liên hệ"
            value={d.deliveryContact}
            onChange={(v) => set((x) => ({ ...x, deliveryContact: v }))}
          />
          <SField
            label="Phone liên hệ"
            value={d.deliveryPhone}
            onChange={(v) => set((x) => ({ ...x, deliveryPhone: v }))}
          />
        </div>
        <SField
          label="Ghi chú khác"
          value={d.deliveryNote}
          onChange={(v) => set((x) => ({ ...x, deliveryNote: v }))}
        />
        <div>
          <label className="text-xs text-zinc-400 mb-1 block">Giao theo thứ tự (chữ đỏ)</label>
          <textarea
            value={d.deliveryOrder}
            onChange={(e) => set((x) => ({ ...x, deliveryOrder: e.target.value }))}
            rows={3}
...
// ── Điểm đánh giá + công nợ (M4) ──

type SupplierRatingItem = {
  id: number;
  poId: number | null;
  poCode: string | null;
  quality: number | null;
    ...
    <div className="mt-2 pt-2 border-t border-zinc-700/50">
      ...
      >
        ...
        <span className="flex items-center gap-1">
          Chất lượng <Stars value={summary.avgQuality} />
        </span>
        <span className="flex items-center gap-1">
          Giao hàng <Stars value={summary.avgDelivery} />
        </span>
        <span className="flex items-center gap-1">
      ...
      {expanded && (
        <div className="mt-2 space-y-1">
          ...
          {summary.ratings.map((r) => (
            <div
              key={r.id}
              className="text-xs text-zinc-400 flex flex-wrap gap-x-3 gap-y-0.5 bg-zinc-900/40 rounded px-2 py-1.5"
            >
              <span className="font-mono text-zinc-300">{r.poCode ?? "—"}</span>
              <span className="flex items-center gap-1">
                CL <Stars value={r.quality} />
              </span>
              <span className="flex items-center gap-1">
                GH <Stars value={r.delivery} />
              </span>
              <span className="flex items-center gap-1">
                Giá <Stars value={r.price} />
              </span>
...
export default function SuppliersTab({ role }: { role: string }) {
  ...
  const startEdit = (s: Supplier) => {
    ...
    setDraft({
      ...
      address: s.address ?? "",
      note: s.note ?? "",
      buyerCompany: s.buyerCompany ?? "",
      buyerProject: s.buyerProject ?? "",
      buyerAddress: s.buyerAddress ?? "",
      buyerRep: s.buyerRep ?? "",
      buyerTitle: s.buyerTitle ?? "",
      buyerPhone: s.buyerPhone ?? "",
      sellerRep: s.sellerRep ?? "",
      receiverCompany: s.receiverCompany ?? "",
      receiverAddress: s.receiverAddress ?? "",
      receiverRep: s.receiverRep ?? "",
        ...
        {suppliers.map((s) => (
          <div key={s.id} className="bg-zinc-800 border border-zinc-700 rounded-xl overflow-hidden">
              ...
              {editingTitle === s.id ? (
                ...
                <>
                  ...
                  {s.title ? (
                    ...
                    <span className="text-xs text-zinc-400 italic flex-1">
            ...
            <div className="p-3">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                    <div className="space-y-0.5">
                      <p className="text-[10px] font-bold text-sky-400 uppercase tracking-wide mb-1">
                        Bên mua
                      </p>
                      {s.buyerCompany && (
                        <p className="text-zinc-200 font-medium">{s.buyerCompany}</p>
                      )}
                      ...
                      {s.buyerRep && (
                        <p className="text-zinc-400">
                          {s.buyerRep}
                          {s.buyerTitle ? ` — ${s.buyerTitle}` : ""}
                        </p>
                      )}
                      {s.buyerPhone && <p className="text-zinc-400">{s.buyerPhone}</p>}
                    </div>
                    <div className="space-y-0.5">
                      <p className="text-[10px] font-bold text-amber-400 uppercase tracking-wide mb-1">
                        Bên bán
                      </p>
                    ...
                    <div className="space-y-0.5">
                      ...
                      {s.receiverCompany && (
                        <p className="text-zinc-200 font-medium">{s.receiverCompany}</p>
                      )}
                      ...
                      {s.receiverSubcon && (
                        <p className="text-zinc-500 italic">NTP: {s.receiverSubcon}</p>
                      )}
                    </div>
                  </div>
                  {canManage && (
              ...
              {editing !== s.id && <SupplierSummaryPanel supplierId={s.id} />}
            </div>
          </div>
        ))}
      </div>
