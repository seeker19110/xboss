...
function Badge({ n, cls }: { n: number; cls: string }) {
  if (!n) return null;
  return (
    <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold leading-none ${cls}`}>
      {n}
    </span>
  );
}
...
export default function ReportsTab({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/materials/reports")
      .then((r) =>
        r.ok ? r.json() : r.json().then((j) => Promise.reject(j?.error ?? "Lỗi server")),
      )
      .then((j) => {
        setReport(j);
        setLoaded(true);
  ...
  if (loading)
    return (
      <div className="text-center py-16 text-zinc-400 text-sm flex items-center justify-center gap-2">
        <RefreshCw className="w-4 h-4 animate-spin" /> Đang tải báo cáo...
      </div>
    );
  if (err)
    return (
    ...
    <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 p-3 bg-orange-950/40 border border-orange-800/50 rounded-lg text-xs text-orange-200">
              ...
              <div>
                ...
                các hạng mục thi công trong vòng 30 ngày tới. Cần đặt hàng ngay để đảm bảo cung ứng
                đúng tiến độ (khuyến nghị nhập trước ít nhất 1 tháng).
              </div>
            </div>
            <div className="overflow-x-auto rounded-xl border border-zinc-800">
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-zinc-800">
                  {r.needsStock.map((m) => {
                    const days = Math.ceil(
                      (new Date(m.earliestStart).getTime() - Date.now()) / 86400000,
                    );
                    return (
                      <tr
                        key={m.id}
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 text-right text-zinc-300">
                          {Number(m.qtyPlanned).toLocaleString("vi")}
                        </td>
                        <td className="py-2.5 px-3 text-right text-zinc-400">
                          {Number(m.qtyUsed).toLocaleString("vi")}
                        </td>
                        <td className="py-2.5 px-3 text-right text-blue-300">
                          {Number(m.qtyStock).toLocaleString("vi")}
                        </td>
                        <td className="py-2.5 px-3 text-right">
                          <span className="font-bold text-orange-300">
                            {Number(m.needQty).toLocaleString("vi")}
                          </span>
                        </td>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              {
                label: "Tổng vật tư",
                value: r.stockSummary.reduce((s, x) => s + Number(x.totalItems), 0),
                cls: "text-zinc-100",
              },
              {
                label: "Tổng tồn kho",
                value: r.stockSummary.reduce((s, x) => s + Number(x.totalStock), 0).toFixed(1),
                cls: "text-blue-300",
              },
              {
                label: "Vượt định mức",
                value: r.stockSummary.reduce((s, x) => s + Number(x.overBudgetCount), 0),
                cls: "text-red-400",
              },
              {
                label: "Cảnh báo tồn thấp",
                value: r.stockSummary.reduce((s, x) => s + Number(x.lowStockCount), 0),
                cls: "text-amber-400",
              },
            ].map((c) => (
              <div
                key={c.label}
                className="bg-zinc-800 border border-zinc-700 rounded-xl p-4 text-center"
              >
            ...
            <table className="w-full text-xs">
              <thead className="bg-zinc-800/60">
                <tr>
                  ...
                  <Th right>Đã dùng</Th>
                  <Th right>Tồn kho</Th>
                  <Th right>% tiêu hao</Th>
                  <Th right>Vượt ĐM</Th>
                  <Th right>Tồn thấp</Th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800">
                {r.stockSummary.map((row) => {
                  const pct =
                    Number(row.totalPlanned) > 0
                      ? (Number(row.totalUsed) / Number(row.totalPlanned)) * 100
                      : 0;
                  return (
                    <tr
                      key={row.sheetCode}
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 text-right text-zinc-300">
                        {Number(row.totalPlanned).toLocaleString("vi")}
                      </td>
                      <td className="py-2.5 px-3 text-right text-zinc-300">
                        {Number(row.totalUsed).toLocaleString("vi")}
                      </td>
                      <td className="py-2.5 px-3 text-right text-blue-300 font-medium">
                        {Number(row.totalStock).toLocaleString("vi")}
                      </td>
                      <td
                        className={`py-2.5 px-3 text-right font-medium ${pct > 100 ? "text-red-400" : pct > 90 ? "text-amber-400" : "text-emerald-400"}`}
                      >
                        {pct.toFixed(1)}%
                      </td>
                      <td className="py-2.5 px-3 text-right">
      ...
      {tab === "overBudget" &&
        (r.overBudget.length === 0 ? (
          <EmptyState message="Không có vật tư nào vượt định mức" />
        ) : (
          <div className="overflow-x-auto rounded-xl border border-zinc-800">
            <table className="w-full text-xs">
              <thead className="bg-zinc-800/60">
      ...
      {tab === "lowStock" &&
        (r.lowStock.length === 0 ? (
          <EmptyState message="Không có vật tư nào dưới mức tối thiểu" />
        ) : (
          <div className="overflow-x-auto rounded-xl border border-zinc-800">
            <table className="w-full text-xs">
              <thead className="bg-zinc-800/60">
              ...
              <tbody className="divide-y divide-zinc-800">
                {r.lowStock.map((x) => (
                  ...
                  >
                    ...
                    <td className="py-2.5 px-3 text-right text-zinc-300">
                      {Number(x.minStockLevel).toLocaleString("vi")}
                    </td>
                    <td className="py-2.5 px-3 text-right text-red-400 font-bold">
                      {(Number(x.minStockLevel) - Number(x.qtyStock)).toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {tab === "aged" &&
        (r.warehouseAge.length === 0 ? (
          <EmptyState message="Không có vật tư tồn kho quá 30 ngày" />
        ) : (
          <div className="overflow-x-auto rounded-xl border border-zinc-800">
            <table className="w-full text-xs">
              <thead className="bg-zinc-800/60">
                <tr>
                  ...
                  <Th right>ĐVT</Th>
                  <Th right>Tồn kho</Th>
                  <Th right>Nhập lần đầu</Th>
                  <Th right>Số ngày tồn</Th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800">
                {r.warehouseAge.map((x) => (
                  ...
                  >
                    ...
                    >
                      {Math.round(Number(x.daysInStock))} ngày
                    </td>
                  </tr>
                ))}
              </tbody>
