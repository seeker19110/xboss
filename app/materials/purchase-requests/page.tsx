...
export default function PurchaseRequestsPage() {
  ...
  useEffect(() => {
    fetchMe().then((user) => {
      if (!user) return;
      setCanApprove(user.role === "admin" || user.role === "pm");
      setUserId(user.id);
    });
    fetch("/api/materials")
      .then((r) => r.json())
  ...
  const submitPR = async () => {
    ...
    if (!Number(addDraft.qtyRequested) || Number(addDraft.qtyRequested) <= 0) {
                ...
                >
                  ...
                  {materials.map((m) => (
                    <option key={m.id} value={m.id}>
                      [{m.sheetCode}] {m.name} {m.unit ? `(${m.unit})` : ""}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div key={pr.id} className="bg-zinc-800 border border-zinc-700 rounded-lg p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {pr.reviewNote && (
                    <div
                      className={`mt-1 text-sm italic ${pr.status === "rejected" ? "text-red-400" : "text-green-400"}`}
                    >
      ...
      {reviewModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-800 border border-zinc-700 rounded-xl w-full max-w-md p-6">
            ...
            {reviewModal.note && (
              <p className="text-zinc-400 text-sm mb-4 italic">Lý do: {reviewModal.note}</p>
            )}
            <div className="mb-4">
