              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                  {[
                    {
                      label: "Tổng vật tư",
                      value: report.stockSummary.reduce((s, r) => s + r.totalItems, 0),
                      cls: "text-zinc-100",
                    },
                    {
                      label: "Tổng tồn kho",
                      value: report.stockSummary.reduce((s, r) => s + r.totalStock, 0).toFixed(1),
                      cls: "text-blue-300",
                    },
                    {
                      label: "Vật tư vượt ĐM",
                      value: report.stockSummary.reduce((s, r) => s + r.overBudgetCount, 0),
                      cls: "text-red-400",
                    },
                    {
                      label: "Cảnh báo tồn thấp",
                      value: report.stockSummary.reduce((s, r) => s + r.lowStockCount, 0),
                      cls: "text-amber-400",
                    },
                  ].map((c) => (
                    <div
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {report.stockSummary.map((row) => {
                        const pct =
                          row.totalPlanned > 0
                            ? ((row.totalUsed / row.totalPlanned) * 100).toFixed(1)
                            : "—";
                        return (
                          <tr
                            key={row.sheetCode}
                            className="border-b border-zinc-700/50 odd:bg-zinc-900/50 even:bg-zinc-800/20 hover:bg-zinc-700/30 transition-colors"
                          >
                            <td className="py-3 pr-4 font-medium text-zinc-100">
                              {row.sheetCode}{" "}
                              <span className="text-zinc-400 text-xs">({row.sheetName})</span>
                            </td>
                            <td className="py-3 pr-4 text-right">{row.totalItems}</td>
                            <td className="py-3 pr-4 text-right text-zinc-300">
                              {row.totalPlanned?.toFixed(1) ?? "0"}
                            </td>
                            <td className="py-3 pr-4 text-right text-zinc-300">
                              {row.totalUsed?.toFixed(1) ?? "0"}
                            </td>
                            <td className="py-3 pr-4 text-right text-blue-300 font-medium">
                              {row.totalStock?.toFixed(1) ?? "0"}
                            </td>
                            <td
                              className={`py-3 pr-4 text-right font-medium ${Number(pct) > 100 ? "text-red-400" : Number(pct) > 90 ? "text-amber-400" : "text-green-400"}`}
                            >
                              {pct}%
                            </td>
                            <td className="py-3 pr-4 text-right">
                              {row.overBudgetCount > 0 ? (
              ...
              <div>
                {report.overBudget.length === 0 ? (
                  <div className="text-center py-16 text-zinc-500">
                    ...
                    <p>Không có vật tư nào vượt định mức</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        {report.overBudget.map((r) => (
                          ...
                          >
                            <td className="py-3 pr-4">
                              ...
                              {r.boqCode && (
                                <div className="text-xs text-zinc-500">{r.boqCode}</div>
                              )}
                            </td>
              ...
              <div>
                {report.lowStock.length === 0 ? (
                  <div className="text-center py-16 text-zinc-500">
                    ...
                    <p>Không có vật tư nào dưới mức tối thiểu</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {report.lowStock.map((r) => (
                          ...
                          >
                            ...
                            <td className="py-3 pr-4 text-zinc-400">{r.sheetCode}</td>
                            <td className="py-3 pr-4 text-right text-zinc-400">{r.unit ?? "-"}</td>
                            <td className="py-3 pr-4 text-right text-amber-400 font-semibold">
                              {r.qtyStock}
                            </td>
                            <td className="py-3 pr-4 text-right text-zinc-300">
                              {r.minStockLevel}
                            ...
                            <td className="py-3 text-right text-red-400 font-semibold">
                              {(r.minStockLevel - r.qtyStock).toFixed(2)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              <div>
                {report.warehouseAge.length === 0 ? (
                  <div className="text-center py-16 text-zinc-500">
                    ...
                    <p>Không có vật tư tồn kho quá 30 ngày</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
              ...
              <div>
                {report.noTaskIssues.length === 0 ? (
                  <div className="text-center py-16 text-zinc-500">
                    ...
                    <p>Mọi lần xuất kho đều có gắn task</p>
                  </div>
                ) : (
                  <>
                        ...
                        <tbody>
                          {report.noTaskIssues.map((r) => (
                            ...
                            >
                              ...
                              <td className="py-3 pr-4 text-right text-orange-400 font-semibold">
                                {r.delta}
                              </td>
