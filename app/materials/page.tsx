...
type ColKey =
  "boqCode" | "stt" | "name" | "unit" | "qtyBoq" | "qtyPlanned" | "diff" | "status" | "note";
...
export default function MaterialsPage() {
  const [activeTab, setActiveTab] = useState<"materials" | "suppliers" | "requests" | "reports">(
    "materials",
  );
  ...
  const load = useCallback(
    (revalidate = false) => {
      ...
      if (
        !revalidate &&
        materialsCache &&
        materialsCache.key === cacheKey &&
        now - materialsCache.ts < CACHE_TTL
      ) {
  ...
  async function api(path: string, init: RequestInit, okFn?: () => void) {
    ...
    okFn?.();
    load(true); // revalidate: bỏ qua cache sau khi mutate
  }
  ...
  async function runSync() {
    if (
      !(await appConfirm(
        "Đồng bộ hai chiều vật tư với Google Sheet?\nThay đổi ở cả hai phía sẽ được gộp (DB ưu tiên khi xung đột).",
    ...
    try {
      ...
      const j = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(j.error ?? "Lỗi đồng bộ Google Sheet");
        return;
      }
  ...
  const removeRows = useCallback(
    async (ids: (number | string)[]) => {
      ...
      if (
        !(await appConfirm(`Xoá ${ids.length} vật tư đang chọn?`, {
          danger: true,
          confirmLabel: "Xoá",
        }))
      )
  ...
  const copyRow = (m: Material) => {
    ...
    if (!sheetTypeId) {
      setError("Vật tư không có hệ — không thể nhân đôi");
      return;
    }
    api("/api/materials", {
      method: "POST",
  ...
  const insertAfter = async (m: Material) => {
    ...
    if (!sheetTypeId) {
      setError("Chọn hệ trước khi chèn hàng");
      return;
    }
    ...
    const values = materials.map((m, i) => {
      switch (key) {
        ...
        case "unit":
          return m.unit ?? "";
        case "qtyBoq":
          return String(m.qtyBoq ?? 0);
        case "qtyPlanned":
          return String(m.qtyPlanned);
        case "diff":
          return String((m.qtyBoq ?? 0) - (m.qtyPlanned ?? 0));
        case "status":
          return BUDGET_LABEL[budgetStatus(m)];
        case "note":
          return m.note ?? "";
        default:
          return "";
  ...
  const addBlankRow = useCallback(async () => {
    const sheetTypeId = sheetFilter
      ? Number(sheetFilter)
      : filtered[filtered.length - 1]?.sheetTypeId;
    if (!sheetTypeId) {
      setError("Chọn hệ (ở góc trên) trước khi thêm dòng mới");
      return;
    }
    await api("/api/materials", {
      method: "POST",
      body: JSON.stringify({ name: "Vật tư mới", sheetTypeId }),
    });
  ...
  const gridColumns: GridColumn<Material>[] = useMemo(() => {
    ...
    const defs: Record<ColKey, GridColumn<Material>> = {
      boqCode: {
        ...
        get: (m) => m.boqCode ?? "",
        toPatch: (raw) => ({ boqCode: raw.trim().toUpperCase() }),
      },
      stt: {
        key: "stt",
        label: colLabels.stt,
      ...
      name: {
        ...
        get: (m) => m.name,
        toPatch: (raw) => (raw.trim() ? { name: raw.trim() } : null),
      },
      unit: {
        key: "unit",
        ...
        get: (m) => m.unit ?? "",
        toPatch: (raw) => ({ unit: raw.trim() || null }),
      },
      qtyBoq: {
        key: "qtyBoq",
  ...
  const stickyCount = useMemo(() => {
    ...
    for (const k of visibleCols) {
      if (["boqCode", "stt", "name"].includes(k)) n++;
      else break;
    }
    return n;
  }, [visibleCols]);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <main className="p-4 sm:p-6 w-full space-y-4">
        ...
        {activeTab === "requests" && (
          <PurchaseRequestsTab role={role} userId={userId} materials={materials} />
        )}
          ...
          <>
                ...
                low.length > 0 && (
                  <div className="rounded-lg border border-amber-800 bg-amber-950 text-amber-200 px-4 py-2.5 text-sm flex items-center gap-2">
                    ...
                    <a
                      ...
                      className="ml-auto underline text-amber-400 hover:text-amber-200 shrink-0"
                    >
                      Xem báo cáo
                    </a>
                  </div>
            ...
            {q && (
              <p className="text-xs text-zinc-500">
                Tìm thấy <span className="text-white font-medium">{filtered.length}</span> /{" "}
                ...
                {filtered.length === 0 && (
                  <p className="p-8 text-center text-zinc-400 text-sm">
                    {q
                      ...
                      : "Chưa có vật tư nào — hãy Import Excel để thêm dữ liệu."}
                  </p>
                )}
              </>
            )}
            ...
            <div
              className={`bg-zinc-900 border border-zinc-800 rounded-xl overflow-auto ${(loading && materials.length === 0) || sheetMode ? "hidden" : ""}`}
              ref={colMenuRef}
                ...
                <thead className="sticky top-0 z-10">
                  <tr className="border-b border-zinc-800 bg-zinc-900">
                    {visibleCols.map((key) => (
                      <th
                        key={key}
                        className={`text-center text-xs text-zinc-400 font-semibold p-0 align-middle whitespace-nowrap relative group/th ${key === "name" ? "w-full min-w-[250px]" : ""}`}
                      >
                        {editingLabel === key ? (
                          ...
                          <div className="flex items-center justify-center gap-1 px-3 py-2.5">
                            <span className={copied === key ? "text-emerald-400" : ""}>
                              {colLabels[key]}
                            </span>
                            <button
                              onClick={() => setColMenu((prev) => (prev === key ? null : key))}
                        ...
                        {colMenu === key && (
                          <div className="absolute top-full left-1/2 -translate-x-1/2 z-50 mt-1 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl py-1 min-w-[160px]">
                            <button
                              onClick={() => copyColToClipboard(key)}
                ...
                <tbody>
                  ...
                  {rowVirtualizer.getVirtualItems().map((vRow) => {
                    ...
                    return (
                      <tr
                        key={m.id}
                        className={`border-b border-zinc-800/50 hover:bg-zinc-700/40 group/row ${mi % 2 === 0 ? "bg-zinc-900" : "bg-zinc-800/30"}`}
                      >
                        {visibleCols.map((key) => (
                          <td
                            key={key}
                            className={`px-3 py-2 align-middle ${key === "name" ? "text-left w-full min-w-[250px] overflow-hidden" : "whitespace-nowrap " + (key === "boqCode" ? "text-left" : "text-center")}`}
                          >
                            {key === "stt" && (
                              <span className="text-zinc-500 text-xs">{globalIdx + 1}</span>
                            )}

                            {key === "boqCode" && (
                              <button
                                onClick={() => canEdit && editMode && editBoq(m)}
                                title={
                                  canEdit && editMode
                                    ? `${m.boqCode ?? "Chưa gán mã BOQ"} — bấm để sửa`
                                    : (m.boqCode ?? "Chưa gán mã BOQ")
                                }
                                  ...
                                  <textarea
                                    ...
                                    onBlur={async (e) => {
                                      ...
                                      const ok = await appConfirm(
                                        `Đổi tên "${m.name}" thành "${newName}"?`,
                                        { confirmLabel: "Đổi tên" },
                                      );
                                      if (ok) {
                                        patch(m.id, { name: newName });
                                      } else {
                                        e.target.value = m.name;
                                      }
                                    }}
                            ...
                            {key === "unit" &&
                              (canEdit && editMode ? (
                                ...
                                >
                                  ...
                                  {m.unit && !DVT_OPTIONS.includes(m.unit) && (
                            ...
                            {key === "qtyBoq" &&
                              (canEdit && editMode ? (
                                <input
                                  type="number"
                                  min="0"
                                  defaultValue={m.qtyBoq ?? 0}
                                  key={`b${m.id}-${m.qtyBoq}`}
                                  onBlur={(e) =>
                                    Number(e.target.value) !== (m.qtyBoq ?? 0) &&
                                    patch(m.id, { qtyBoq: Number(e.target.value) })
                                  }
                            ...
                            {key === "qtyPlanned" &&
                              (canEdit && editMode ? (
                                <input
                                  ...
                                  key={`p${m.id}-${m.qtyPlanned}`}
                                  onBlur={(e) =>
                                    Number(e.target.value) !== m.qtyPlanned &&
                                    patch(m.id, { qtyPlanned: Number(e.target.value) })
                                  }
                            ...
                            {key === "diff" && (
                              <span
                                className={`font-medium text-xs ${!hasBothQty ? "text-zinc-600" : diff > 0 ? "text-emerald-400" : diff < 0 ? "text-red-400" : "text-zinc-400"}`}
                              >
                                {!hasBothQty
                                  ? "—"
                                  : diff > 0
                              ...
                              (canEdit && editMode ? (
                                <input
                                  ...
                                  onBlur={(e) =>
                                    ...
                                    patch(m.id, { note: e.target.value || null })
                                  }
                                  placeholder="—"
                          ...
                          <td className="px-1 py-2 whitespace-nowrap">
                            <div className="flex items-center gap-0 opacity-100 sm:opacity-0 sm:group-hover/row:opacity-100 transition-opacity">
                              <button
                                ...
                                className="text-zinc-500 hover:text-emerald-400 p-2"
                              >
            ...
            {syncResult.conflicts.length > 0 && (
              <div>
                <div className="text-xs font-medium text-amber-400 mb-1.5 flex items-center gap-1">
                  <AlertTriangle className="w-3.5 h-3.5" /> {syncResult.conflicts.length} xung đột
                  (DB ưu tiên)
                </div>
            ...
            {syncResult.skipped.length > 0 && (
              <div>
                <div className="text-xs font-medium text-zinc-400 mb-1.5">
                  {syncResult.skipped.length} dòng bỏ qua
                </div>
          ...
          <div className="p-5 space-y-3">
                  ...
                  {matches.map((m) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isDupe && (
                          <span className="ml-auto text-red-400 font-medium shrink-0">Trùng!</span>
                        )}
                      </button>
                    );
                  })}
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-sm text-zinc-400 hover:text-white"
              >
                Huỷ
              </button>
              <button
...
function MaterialHistoryModal({
  ...
  const load = useCallback(() => {
    fetch(`/api/materials/${material.id}/transactions`)
      .then((r) => r.json())
      .then((j) => setItems(j.transactions ?? []));
  }, [material.id]);
  useEffect(() => {
    load();
  }, [load]);
      ...
      <div className="px-4 py-3 border-b border-zinc-800 flex items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-zinc-500">
            ...
            {" · "} Đã dùng{" "}
            <span className={over ? "text-red-400 font-medium" : "text-zinc-300"}>{qtyUsed}</span>
      ...
      {canEdit && (
        <div className="px-4 py-3 border-b border-zinc-800 bg-zinc-950/50">
          ...
          <div className="flex gap-2">
            ...
            />
            <input
              placeholder="Ghi chú (vd: xuất cho tầng 24F)"
              value={note}
            ...
            <button
              onClick={() => add(-1)}
              disabled={busy || !Number(delta)}
              className="bg-zinc-700 hover:bg-zinc-600 disabled:opacity-40 rounded-lg px-3 py-1.5 text-sm font-medium"
            >
              −
            </button>
          </div>
            ...
            <tbody>
              {items.map((t) => (
                <tr key={t.id} className="border-b border-zinc-800/50">
                  ...
                  <td
                    className={`py-1.5 pr-2 text-right font-medium ${t.delta >= 0 ? "text-emerald-400" : "text-amber-400"}`}
                  >
