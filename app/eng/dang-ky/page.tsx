...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const r = await fetch('/api/eng/auth/register', {
        method: 'POST',
    ...
    <div className="min-h-[70vh] flex items-center justify-center">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400 mt-1 text-sm">Bắt đầu hành trình học tiếng Anh của bạn</p>
        </div>

        <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1.5">Họ tên</label>
            <input
              type="text"
              value={name}
          ...
          <p className="text-xs text-zinc-500 text-center">
            Bằng cách đăng ký, bạn có thể học miễn phí ngay lập tức.
          </p>
        </form>

        <p className="text-center text-zinc-500 text-sm mt-5">
          Đã có tài khoản?{' '}
          <Link href="/eng/dang-nhap" className="text-indigo-400 hover:text-indigo-300 font-medium">
            Đăng nhập
          </Link>
        </p>
