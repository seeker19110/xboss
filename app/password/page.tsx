...
export default function PasswordPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg(
        res.ok
          ? { ok: true, text: "Đã đổi mật khẩu thành công" }
          : { ok: false, text: j.error ?? "Lỗi không xác định" },
      );
      if (res.ok) {
        setOld("");
        setNew("");
      ...
      setMsg({ ok: false, text: "Mất kết nối mạng — vui lòng thử lại" });
    } finally {
      setBusy(false);
    }
  }
      ...
      <main className="p-6 max-w-sm mx-auto">
        ...
        >
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full bg-emerald-700 hover:bg-emerald-800 disabled:bg-zinc-800 py-2.5 rounded-lg font-medium text-sm transition text-on-accent"
          >
