...
export default function OrderContent({ isEmbed = false }: { isEmbed?: boolean }) {
  ...
  const InfoField = ({ label, value, onChange, placeholder = '', labelWidth = '108px' }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; labelWidth?: string }) => (
    <div className="flex items-baseline gap-2 text-[10px] min-w-0 py-[2px]">
      <span className="shrink-0 text-gray-600 whitespace-nowrap" style={labelWidth !== 'auto' ? { width: labelWidth } : undefined}>{label}</span>
          ...
          <div className="mb-3 space-y-2 text-[10px]">
            <div className="bg-sky-50 border border-sky-200 rounded px-3 py-2 space-y-1">
              ...
              <div className="grid gap-y-1 [&>*]:min-w-0" style={{ gridTemplateColumns: '62% 35%', columnGap: '3%' }}>
                ...
                <InfoField label="Đại diện:" value={buyer.rep} onChange={v => setBuyer(b => ({ ...b, rep: v }))} placeholder="…" labelWidth="auto" />
                <InfoField label="Tên công trình:" value={buyer.project} onChange={v => setBuyer(b => ({ ...b, project: v }))} placeholder="…" />
                <InfoField label="Chức vụ:" value={buyer.title} onChange={v => setBuyer(b => ({ ...b, title: v }))} placeholder="…" labelWidth="auto" />
                <InfoField label="Địa chỉ giao hàng:" value={buyer.address} onChange={v => setBuyer(b => ({ ...b, address: v }))} placeholder="…" />
                <InfoField label="Phone:" value={buyer.phone} onChange={v => setBuyer(b => ({ ...b, phone: v }))} placeholder="…" labelWidth="auto" />
              </div>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded px-3 py-2 space-y-1">
              ...
              <div className="grid gap-y-1 [&>*]:min-w-0" style={{ gridTemplateColumns: '62% 35%', columnGap: '3%' }}>
                ...
                <InfoField label="Đại diện:" value={seller.rep} onChange={v => setSeller(s => ({ ...s, rep: v }))} placeholder="…" labelWidth="auto" />
                <InfoField label="Địa chỉ:" value={seller.address} onChange={v => setSeller(s => ({ ...s, address: v }))} placeholder="…" />
                <InfoField label="Phone:" value={seller.phone} onChange={v => setSeller(s => ({ ...s, phone: v }))} placeholder="…" labelWidth="auto" />
              </div>
            </div>
            <div className="bg-green-50 border border-green-200 rounded px-3 py-2 space-y-1">
              ...
              <div className="grid gap-y-1 [&>*]:min-w-0" style={{ gridTemplateColumns: '62% 35%', columnGap: '3%' }}>
                ...
                <InfoField label="Đại diện:" value={receiver.rep} onChange={v => setReceiver(r => ({ ...r, rep: v }))} placeholder="…" labelWidth="auto" />
                <InfoField label="Địa chỉ giao hàng:" value={receiver.address} onChange={v => setReceiver(r => ({ ...r, address: v }))} placeholder="…" />
                <InfoField label="Phone:" value={receiver.phone} onChange={v => setReceiver(r => ({ ...r, phone: v }))} placeholder="…" labelWidth="auto" />
