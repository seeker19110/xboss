...
export default function SpreadsheetGrid<Row>({
  ...
  useEffect(() => {
    setActive((a) => ({
      r: Math.min(a.r, Math.max(0, nRows - 1)),
      c: Math.min(a.c, Math.max(0, nCols - 1)),
    }));
  }, [nRows, nCols]);
  ...
  const commitCells = useCallback(
    async (cells: { r: number; c: number; raw: string }[], track = true) => {
      ...
      const edits: GridEdit[] = [...byRow.entries()].map(([k, patch]) => ({
        rowId: rowIdMap.get(k)!,
        patch,
      }));

      setSaving((prev) => {
        const n = new Set(prev);
        touched.forEach((k) => n.add(k));
        return n;
      });
      setErrors((prev) => {
        const n = new Map(prev);
        touched.forEach((k) => n.delete(k));
        return n;
      });
      try {
  ...
  const startEdit = useCallback(
    (preset?: string) => {
      ...
      if (col.type === "checkbox") {
        // checkbox: toggle ngay, không vào edit
  ...
  const pasteAt = useCallback(
    async (text: string) => {
      ...
      const cells = spreadPaste(matrix, active.r, active.c).filter(
        ({ r, c }) => r < nRows && c < nCols,
      );
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case "Tab":
        e.preventDefault();
        move(0, e.key === "Tab" && e.shiftKey ? -1 : 1, false);
        break;
      case "Enter":
      case "F2":
        e.preventDefault();
  ...
  const doReplace = useCallback(() => {
    ...
    for (let r = 0; r < sortedAndFiltered.length; r++) {
      ...
      for (let c = 0; c < columns.length; c++) {
        ...
        if (val.includes(q)) {
          const newVal = String(columns[c]?.get(row) ?? "").replace(
  ...
  return (
    <div className="relative">
      ...
      {!ro && (
        <div className="bg-zinc-900/50 border-b border-zinc-800 space-y-2 px-3 py-2">
          ...
          <div className="flex items-center gap-1 flex-wrap text-zinc-300 text-lg">
            ...
            <ToolBtn onClick={redo} disabled={!redoStack.length} title="Làm lại (Ctrl+Y)">
              ↷
            </ToolBtn>
            <span className="w-px h-4 bg-zinc-700 mx-1" />
            <ToolBtn onClick={() => setColAlign("left")} title="Căn trái cột">
              ⇤
            </ToolBtn>
            <ToolBtn onClick={() => setColAlign("center")} title="Căn giữa cột">
              ↔
            </ToolBtn>
            <ToolBtn onClick={() => setColAlign("right")} title="Căn phải cột">
              ⇥
            </ToolBtn>
            <span className="w-px h-4 bg-zinc-700 mx-1" />
            <ToolBtn
              onClick={() => setFreezeCols((n) => Math.max(0, n - 1))}
              disabled={freezeCols <= 0}
              title="Bớt cột đóng băng"
            >
              ❄−
            </ToolBtn>
            <span className="text-sm text-zinc-400 px-1">đóng băng {freezeCols} cột</span>
            <ToolBtn
              onClick={() => setFreezeCols((n) => Math.min(nCols, n + 1))}
              disabled={freezeCols >= nCols}
              title="Thêm cột đóng băng"
            >
              ❄+
            </ToolBtn>
            {(onAddRow || onDeleteRows) && <span className="w-px h-4 bg-zinc-700 mx-1" />}
            {onAddRow && (
              <ToolBtn onClick={() => onAddRow()} title="Thêm dòng mới">
                ＋ dòng
              </ToolBtn>
            )}
            {onDeleteRows && (
            ...
            {Object.keys(colFilters).some((c) => colFilters[Number(c)]?.size) && (
              <button
                ...
                className="ml-1 text-xs text-sky-400 hover:text-sky-300"
              >
                Xoá lọc
              </button>
            )}
      ...
      <div
        ...
        onPaste={
          ro
            ...
            : (e) => {
                e.preventDefault();
                pasteAt(e.clipboardData.getData("text/plain"));
              }
        }
      ...
      >
        {/* table/thead/tbody để role="presentation" + tr/th/td gán role tường minh:
              ...
              return (
                <tr key={rowKey(row)} role="row">
                  {columns.map((col, c) => {
                    ...
                    // Mặc định căn giữa mọi cột; vẫn tôn trọng ghi đè per-cột
                        ...
                        {editing && isActive ? (
                          ...
                          <span className={wrap ? "block" : "block overflow-hidden text-ellipsis"}>
                            {col.type === "checkbox"
                              ...
                              : col.render
                                ? col.render(row)
                                : displayValue(col.get(row))}
                          </span>
                        )}
                        {isSaving && (
                          <Loader2
                            className="w-9 h-9 animate-spin text-sky-400 absolute right-1 top-1/2 -translate-y-1/2"
                            aria-label="Đang lưu"
                          />
                        )}
                      </td>
      ...
      {selectionStats.count > 1 && (
        <div className="flex items-center gap-4 justify-end bg-zinc-900/50 border-t border-zinc-800 px-3 py-1 text-[11px] text-zinc-400">
          <span>
            Đếm: <b className="text-zinc-200">{selectionStats.count}</b>
          </span>
          {selectionStats.numCount > 0 && (
            <>
              <span>
                Tổng: <b className="text-zinc-200">{selectionStats.sum.toLocaleString("vi")}</b>
              </span>
              <span>
                TB:{" "}
                <b className="text-zinc-200">
                  {selectionStats.avg.toLocaleString("vi", { maximumFractionDigits: 2 })}
                </b>
              </span>
              <span>
                Số ô số: <b className="text-zinc-200">{selectionStats.numCount}</b>
              </span>
            </>
          )}
        </div>
          ...
          {!ro && (
            <>
              ...
              {clipboard && (
                <button
                  onClick={() => {
                    pasteAt(serializeTSV(clipboard.matrix));
                    setContextMenu(null);
                  }}
                  className="w-full flex items-center gap-2 px-3 py-1.5 text-sm text-zinc-300 hover:bg-zinc-700 text-left"
                >
    ...
    >
      ...
      <div className="max-h-48 overflow-y-auto">
        {shown.map((v) => (
          <label
            key={v}
            className="flex items-center gap-2 px-1 py-1 text-xs cursor-pointer hover:bg-zinc-700 rounded"
          >
            <input
              type="checkbox"
      ...
      <div className="flex justify-end gap-2 mt-2 pt-2 border-t border-zinc-700">
        <button onClick={onClose} className="text-xs text-zinc-400 hover:text-white px-2 py-1">
          Huỷ
        </button>
        <button
...
function CellEditor<Row>({
  ...
  const base =
    "absolute inset-0 w-full h-full px-2 bg-zinc-900 text-white outline-none ring-1 ring-inset ring-sky-400 select-text";
