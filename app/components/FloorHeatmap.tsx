...
function TowerTable({ tower, byKey }: { tower: Tower; byKey: Map<string, CellData> }) {
  return (
    <table className="border-separate" style={{ borderSpacing: 3 }}>
      <thead>
        <tr>
          <th className="text-xs text-zinc-400 font-normal text-right pr-2 sticky left-0 bg-zinc-900">
            Tầng
          </th>
          {tower.sheets.map((s) => (
            <th
              key={s}
              className="text-[11px] text-zinc-400 font-medium px-1 pb-1 whitespace-nowrap min-w-[88px]"
            >
              {s}
            </th>
          ))}
      ...
      <tbody>
        {tower.floors.map((f) => (
          <tr key={f}>
            <td className="text-xs text-zinc-400 text-right pr-2 font-mono sticky left-0 bg-zinc-900">
              {f}
            </td>
            {tower.sheets.map((s) => {
              ...
              if (!c)
                return (
                  <td
                    key={s}
                    className="rounded bg-zinc-900 border border-dashed border-zinc-800 text-center text-[10px] text-zinc-700 h-8"
                  >
                    —
                  </td>
                );
              ...
              const inner = (
                <div className="flex items-center justify-center gap-1 h-8 px-2 text-xs font-medium">
                  ...
                  {c.delayed > 0 && (
                    <span className="text-[10px] leading-none bg-red-600/80 text-white rounded px-1">
                      {c.delayed}
                    </span>
                  )}
                </div>
              ...
              return (
                <td
                  key={s}
                  className={`rounded text-center transition hover:scale-105 hover:z-10 ${cellClass(c.progress, c.delayed)}`}
                  title={`${s} · ${f}: ${Math.round(c.progress * 100)}% (${c.tasks} task${c.delayed ? `, ${c.delayed} trễ` : ""})`}
                >
                  {slug ? (
                    <a href={`/tracking/${slug}?floor=${encodeURIComponent(f)}`} className="block">
                      {inner}
                    </a>
                  ) : (
                    inner
...
export default function FloorHeatmap() {
  ...
  const byKey = useMemo(
    () =>
      new Map((data?.cells ?? []).map((c) => [`${c.tower ?? ""}|${c.floor}|${c.sheetType}`, c])),
    [data],
  );
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-8">
      ...
      <div
        className={multi ? "grid gap-4" : "overflow-auto"}
        style={multi ? { gridTemplateColumns: `repeat(${towers.length}, minmax(0, 1fr))` } : {}}
      >
        {multi ? (
          towers.map((t) => {
