...
type CellData = { sheetType: string; floor: string; progress: number; tasks: number; delayed: number };
type Data = { floors: string[]; sheets: string[]; cells: CellData[] };
...
export default function FloorHeatmap() {
  ...
  const byKey = new Map(data.cells.map(c => [`${c.floor}|${c.sheetType}`, c]));
