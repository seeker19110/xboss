  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-8">
      ...
      <p className="text-xs text-zinc-400 mb-3">
        Ngoại suy từ tốc độ cập nhật thực tế {data.windowDays} ngày gần nhất — cập nhật tiến độ càng
        đều, dự báo càng chính xác
      </p>
              ...
              {done ? (
                ...
                <>
                  ...
                  <p className="text-xs text-zinc-400">
                    còn ~{f.daysLeft} ngày · {(f.ratePerWeek * 100).toFixed(1)}%/tuần
                  </p>
                  {f.lateDays !== null && (
                    <p
                      className={`text-xs mt-1 font-medium ${f.lateDays > 0 ? "text-red-400" : "text-emerald-400"}`}
                    >
                      {f.lateDays > 0
                        ? `⚠ trễ deadline ~${f.lateDays} ngày`
                        : `✓ sớm hơn deadline ${-f.lateDays} ngày`}
                    </p>
                  )}
                </>
              ) : (
                <>
                  ...
                  <p className="text-[11px] text-zinc-400 mt-1">
                    Cần cập nhật tiến độ thường xuyên trong {data.windowDays} ngày để có dự báo
                  </p>
                </>
              )}
              {f.deadline && (
                <p className="text-[11px] text-zinc-400 mt-1">Deadline: {fmt(f.deadline)}</p>
              )}
            </div>
          );
        })}
