  ...
  if (editing) {
    return (
      <span className="inline-flex items-center gap-1 align-middle">
        ...
        />
        <button onClick={e => { e.preventDefault(); e.stopPropagation(); saveText(tkey, draft, def); setEditing(false); }}
          title="Lưu" className="text-emerald-400 hover:text-emerald-300"><Check className="w-3.5 h-3.5" /></button>
        <button onClick={e => { e.preventDefault(); e.stopPropagation(); setEditing(false); }} title="Huỷ"
          className="text-zinc-500 hover:text-zinc-300"><X className="w-3.5 h-3.5" /></button>
        {value !== def && (
          <button onClick={e => { e.preventDefault(); e.stopPropagation(); saveText(tkey, '', def); setEditing(false); }}
