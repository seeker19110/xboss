...
// GET /api/hse?kind=&month=(YYYY-MM) — sổ HSE. Mọi vai trò đăng nhập xem được.
...
// càng nhiều báo cáo càng tốt, không chặn).
  ...
  const id = await insertId(
    `INSERT INTO hse_records
       (kind, record_date, floor_label, area, description, severity, permit_type,
        permit_from, permit_to, action_required, action_assignee, action_due, action_status, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    input.kind,
    input.recordDate,
    input.floorLabel,
