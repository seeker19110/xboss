...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const sets: string[] = [];
  const args: unknown[] = [];
  if (b.unit !== undefined) { sets.push("unit = ?"); args.push((String(b.unit).trim() || null)); }
  if (b.quantity !== undefined) {
    const q = b.quantity === "" || b.quantity == null ? null : Number(b.quantity);
    sets.push("quantity = ?"); args.push(q != null && Number.isFinite(q) && q >= 0 ? q : null);
  }
  if (b.labor !== undefined) {
    const l = b.labor === "" || b.labor == null ? null : Number(b.labor);
    sets.push("labor = ?"); args.push(l != null && Number.isFinite(l) && l >= 0 ? l : null);
  }
  if (b.note !== undefined) { sets.push("note = ?"); args.push((String(b.note).trim() || null)); }
