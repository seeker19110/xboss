...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ items: await listEngineeringSources(ctx.projectId, limit) });
}
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ item: await createEngineeringSource(parsed.data, ctx.user.id) }, { status: 201 });
}
