...
export async function POST(
  ...
  const approval = await queryOne<{ id: number; sheetTypeId: number; floorLabel: string }>(
    `SELECT id, sheet_type_id AS "sheetTypeId", floor_label AS "floorLabel" FROM floor_approvals WHERE id = ?`,
    approvalId,
  );
  if (!approval)
    return NextResponse.json({ error: "Không tìm thấy bản ghi nghiệm thu" }, { status: 404 });
  ...
  if (ct.includes("application/json")) {
    ...
    if (!url || !/^https?:\/\//i.test(url))
      return NextResponse.json(
        { error: "URL không hợp lệ (phải bắt đầu bằng http/https)" },
        { status: 400 },
      );
