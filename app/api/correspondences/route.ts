...
export async function GET(req: NextRequest) {
  ...
  const correspondences = await listCorrespondences({
...
// POST /api/correspondences — tạo văn bản mới (Admin/PM/kỹ sư).
  ...
  const id = await insertId(
    `INSERT INTO correspondences
       (code, direction, kind, counterparty, subject, sent_date, due_date, status,
        task_id, work_package_id, drawing_id, note, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    input.code,
    input.direction,
    input.kind,
