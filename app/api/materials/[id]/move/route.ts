...
export async function PATCH(
  ...
  if (!canEditMaterials(user.role))
    return NextResponse.json({ error: "Chỉ Admin/PM/Kỹ sư mới di chuyển được" }, { status: 403 });
  ...
  const cur =
    projectId != null
      ? await queryOne<{ sort_order: number; sheet_type_id: number | null }>(
          `SELECT sort_order, sheet_type_id FROM materials WHERE id = ? AND project_id = ?`,
          id,
          projectId,
        )
      : undefined;
  ...
  if (cur.sheet_type_id != null) {
    neighbor = await queryOne<{ id: number; sort_order: number }>(
      dir === "up"
        ? `SELECT id, sort_order FROM materials WHERE sheet_type_id = ? AND project_id = ? AND sort_order < ? ORDER BY sort_order DESC LIMIT 1`
        : `SELECT id, sort_order FROM materials WHERE sheet_type_id = ? AND project_id = ? AND sort_order > ? ORDER BY sort_order ASC LIMIT 1`,
      cur.sheet_type_id,
      projectId,
      cur.sort_order,
    );
    ...
    neighbor = await queryOne<{ id: number; sort_order: number }>(
      dir === "up"
        ? `SELECT id, sort_order FROM materials WHERE sheet_type_id IS NULL AND project_id = ? AND sort_order < ? ORDER BY sort_order DESC LIMIT 1`
        : `SELECT id, sort_order FROM materials WHERE sheet_type_id IS NULL AND project_id = ? AND sort_order > ? ORDER BY sort_order ASC LIMIT 1`,
      projectId,
      cur.sort_order,
    );
  }
