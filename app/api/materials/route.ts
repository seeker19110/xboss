...
// POST /api/materials  body: { sheetTypeId, name, unit?, qtyPlanned?, note? } (Admin/PM/Kỹ sư).
export async function POST(req: NextRequest) {
  ...
  const id = await insertId(
    `INSERT INTO materials (sheet_type_id, name, unit, qty_planned, qty_used, status, note)
     VALUES (?, ?, ?, ?, 0, 'dat_hang', ?)`,
    sheetTypeId, name, body.unit ? String(body.unit).trim() : null,
