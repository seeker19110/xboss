...
// GET /api/meetings/:id — chi tiết 1 cuộc họp kèm action.
...
export async function PATCH(
  ...
  const meeting = await queryOne<{ id: number; created_by: number | null }>(
    `SELECT id, created_by FROM meetings WHERE id = ?`,
    id,
  );
  if (!meeting) return NextResponse.json({ error: "Không tìm thấy cuộc họp" }, { status: 404 });
...
export async function DELETE(
  ...
  const r = await run(`DELETE FROM meetings WHERE id = ?`, id);
  if (r.changes === 0)
