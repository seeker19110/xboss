...
type PhotoRow = { id: number; file_name: string; mime_type: string; uploaded_by: number | null; task_id: number };
...
export async function GET(_req: NextRequest, { params: paramsP }: { params: Promise<{ id: string }> }) {
  ...
  const photo = await queryOne<PhotoRow>(
    `SELECT id, file_name, mime_type, uploaded_by, task_id FROM task_photos WHERE id = ?`, id);
  if (!photo) return NextResponse.json({ error: "Không tìm thấy ảnh" }, { status: 404 });

  if (!(await canTouchTask(user, photo.task_id)))
