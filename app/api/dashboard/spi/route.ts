...
export async function GET() {
  ...
  let oPlanned = 0, oActual = 0, oCount = 0;
  ...
  for (const r of rows) {
    ...
    g.plannedSum += planned; g.actualSum += actual; g.count += 1;
    groups.set(r.sheetType, g);
    oPlanned += planned; oActual += actual; oCount += 1;
  }
  ...
  const make = (plannedSum: number, actualSum: number, count: number) => {
    ...
    const planned = plannedSum / count, actual = actualSum / count;
    return {
  ...
  const spi = [...groups.entries()].map(([sheetType, g]) => ({
    sheetType, ...make(g.plannedSum, g.actualSum, g.count),
  }));
