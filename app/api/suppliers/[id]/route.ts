...
const FIELD_MAP: Record<string, string> = {
  ...
  receiverRep: "receiver_rep",
  receiverPhone: "receiver_phone",
  receiverSubcon: "receiver_subcon",
...
export async function PATCH(
  ...
  const s = await queryOne(`SELECT id FROM suppliers WHERE id = ? AND org_id = ?`, id, user.orgId);
...
export async function DELETE(
  ...
  if (used)
    return NextResponse.json(
      { error: "Nhà cung cấp đang có đơn hàng, không thể xoá" },
      { status: 409 },
    );
