...
export async function POST(
  ...
  const newFloor =
    body.floorLabel === undefined
      ? src.floor_label
      : body.floorLabel === null
        ? null
        : String(body.floorLabel).trim() || null;
  ...
  if (dup)
    return NextResponse.json({ error: `Mã "${newCode}" đã tồn tại trong sheet` }, { status: 409 });
  ...
  const srcTasks = await query<{
    ...
    `SELECT id, code, name, sort_order, start_date, end_date, assigned_to, assigned_manual, drawing_url
      FROM tasks WHERE package_id = ? ORDER BY sort_order`,
    srcId,
  );

  // Bọc sort_order bump + INSERT nhóm + INSERT tasks/dims trong 1 transaction.
  // 23505 từ unique index (sheet_type_id, lower(code)) bắt race 2 người dùng cùng copy
  ...
  try {
    ...
    if ((err as { code?: string }).code === "23505")
      return NextResponse.json(
        { error: `Mã "${newCode}" đã tồn tại trong sheet` },
        { status: 409 },
      );
    throw err;
  }
  ...
  return NextResponse.json(
    { id: newPkgId, code: newCode, name: newName, tasks: srcTasks.length },
    { status: 201 },
  );
}
