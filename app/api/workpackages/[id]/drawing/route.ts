...
import { ensureUploadDir, newDrawingFileName, photoPath, MAX_DOC_BYTES, extForDocMime } from "@/lib/photos";
...
type WP = { id: number; drawingUrl: string | null; drawingFileName: string | null; drawingOriginalName: string | null };
...
export async function GET(_req: NextRequest, { params: paramsP }: { params: Promise<{ id: string }> }) {
  ...
  if (!wp.drawingFileName) return NextResponse.json({ error: "Chưa có file bản vẽ" }, { status: 404 });
...
export async function POST(req: NextRequest, { params: paramsP }: { params: Promise<{ id: string }> }) {
  ...
  const wp = await queryOne<{ id: number; drawingFileName: string | null }>(
    `SELECT id, drawing_file_name AS "drawingFileName" FROM work_packages WHERE id = ?`, id);
  if (!wp) return NextResponse.json({ error: "Không tìm thấy nhóm" }, { status: 404 });
  ...
  await run(
    ...
    drawingUrl, fileName, file.name || null, id);
...
export async function DELETE(_req: NextRequest, { params: paramsP }: { params: Promise<{ id: string }> }) {
  ...
  const wp = await queryOne<{ drawingFileName: string | null }>(
    `SELECT drawing_file_name AS "drawingFileName" FROM work_packages WHERE id = ?`, id);

  await run(`UPDATE work_packages SET drawing_url = NULL, drawing_file_name = NULL, drawing_original_name = NULL WHERE id = ?`, id);
