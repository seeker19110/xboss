...
export async function PATCH(
  ...
  if (dir !== "left" && dir !== "right")
    return NextResponse.json({ error: "direction phải là 'left' hoặc 'right'" }, { status: 400 });

  // Lấy task đầu tiên làm mẫu để đọc sort_order.
  const firstTask = await queryOne<{ id: number }>(
    `SELECT id FROM tasks WHERE package_id = ? ORDER BY sort_order, id LIMIT 1`,
    pkgId,
  );
