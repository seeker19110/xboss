...
export async function POST(
  ...
  if (body.length > 2000)
    return NextResponse.json({ error: "Bình luận quá dài (tối đa 2000 ký tự)" }, { status: 413 });
  ...
  const related = await query<{ uid: number }>(
    `SELECT DISTINCT user_id AS uid FROM task_comments WHERE task_id = ? AND user_id IS NOT NULL
     UNION SELECT assigned_to FROM tasks WHERE id = ? AND assigned_to IS NOT NULL`,
    taskId,
    taskId,
  );
  ...
  for (const uid of recipients) {
    ...
    await run(
      ...
      uid,
      taskId,
      `💬 ${user.name} bình luận ${task.code}: ${preview}`,
    );
  }
  ...
  if (recipients.length > 0) {
    ...
    await sendPushToUsers(recipients, {
      ...
      /* push lỗi không được chặn việc lưu bình luận */
    });
  }
  ...
  const comment = await queryOne(
    `SELECT c.id, c.body, c.created_at AS "createdAt",
            c.user_id AS "userId", u.name AS "userName", u.role AS "userRole"
       FROM task_comments c LEFT JOIN users u ON c.user_id = u.id WHERE c.id = ?`,
    id,
  );
