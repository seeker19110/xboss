...
export default function PrintPage() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center text-zinc-500 text-sm">Đang tải...</div>
  );
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      ...
      {/* ── Trang in ── */}
      <div className="print-page mx-auto px-8 py-6" style={{ maxWidth: 960, fontFamily: 'Arial, sans-serif', fontSize: 13 }}>
        ...
        <div className="flex items-start gap-6 mb-4">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-sm">
              Công trình: <strong>{project.name ?? '...'}{project.tower ? ` (${project.tower})` : ''}</strong>
            </p>
            {project.contractor && (
              <p className="text-sm">Gói thầu: <strong>{project.contractor}</strong></p>
            )}
        ...
        {/* ── Bảng chính ── */}
        <table className="w-full border-collapse text-[12px]" style={{ borderCollapse: 'collapse' }}>
          <thead>
          ...
          <tbody>
            ...
            {billRows.length === 0 && (
              <tr><td className="border border-black px-2 py-1.5 text-zinc-400 italic" colSpan={7}>Chưa có khoản thanh toán</td></tr>
            )}
            {billRows.map(b => {
              stt++;
              return (
            ...
            {/* ── Dòng tổng GTTHTC ── */}
            <SummaryRow code="GTTHTC" label="TỔNG GIÁ TRỊ CÔNG VIỆC HOÀN THÀNH (CHƯA VAT)" unit="TC" value={gtthtc} bold />
            ...
            {advances.map((b, i) => (
              <tr key={b.id} className="bg-[#fff2cc]">
                <td className="border border-black px-1 py-1.5 text-center font-bold text-[11px]">{i === 0 ? '(TU)' : ''}</td>
                <td className="border border-black px-2 py-1.5 font-bold">
                  {i === 0 ? 'TẠM ỨNG' : ''}{b.description ? ` — ${b.description}` : ''}
                  {b.period ? ` [${b.period}]` : ''}
                </td>
                <td className="border border-black px-1 py-1.5" />
                <td className="border border-black px-1 py-1.5" />
                <td className="border border-black px-2 py-1.5" />
                <td className="border border-black px-2 py-1.5 text-right font-semibold text-violet-700">{fmtVND(b.amount)}</td>
                <td className="border border-black px-2 py-1.5 text-[11px]">{b.note ?? ''}</td>
              </tr>
            ))}
            {advances.length === 0 && (
              <SummaryRow code="(TU)" label="TẠM ỨNG" value={0} />
            )}
            ...
            {/* ── Tổng được TT kỳ này ── */}
            <SummaryRow code="(GTTTK)" label="TỔNG GIÁ TRỊ ĐƯỢC THANH TOÁN ĐẾN KỲ NÀY" value={gtttk} bold />

            {/* ── Tổng đã TT kỳ trước ── */}
            <SummaryRow code="(GTTTKT)" label="TỔNG GIÁ TRỊ ĐÃ THANH TOÁN ĐẾN KỲ TRƯỚC (GỒM TẠM ỨNG)" value={0} />

            {/* ── Đề nghị TT ── */}
            <SummaryRow code="(DNTT)" label="ĐỀ NGHỊ THANH TOÁN (CÓ VAT)" value={gtttk} bold highlight />
          </tbody>
        </table>
...
function SectionHeader({ label, title }: { label: string; title: string }) {
  return (
    <tr className="bg-[#d9e1f2]">
      ...
      <td className="border border-black px-2 py-1.5 font-bold" colSpan={6}>{title}</td>
    </tr>
  );
}

function SummaryRow({ code, label, unit, value, bold, highlight }: {
  code: string; label: string; unit?: string; value: number; bold?: boolean; highlight?: boolean;
}) {
  return (
    <tr className={highlight ? 'bg-[#e2efda]' : 'bg-[#fff2cc]'}>
      <td className={`border border-black px-1 py-1.5 text-center text-[11px] ${bold ? 'font-bold' : ''}`}>{code}</td>
      ...
      <td className="border border-black px-1 py-1.5" />
      <td className="border border-black px-2 py-1.5" />
      <td className={`border border-black px-2 py-1.5 text-right ${bold ? 'font-bold' : ''} ${highlight ? 'text-green-800' : ''}`}>
        {fmtVND(value)}
      </td>
      <td className="border border-black px-2 py-1.5" />
    </tr>
