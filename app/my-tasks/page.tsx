...
type ActivityEvent = {
  type: EventType; taskId: number; taskCode: string; taskName: string;
  detail: string; by: string | null; at: string; sheetSlug: string | null;
};
type SheetActivity = { sheetCode: string; sheetName: string; sheetSlug: string | null; events: ActivityEvent[] };
type Feed = {
  overdue: FeedTaskItem[]; dueSoon: FeedTaskItem[]; upcomingStart: FeedTaskItem[];
  recentActivity: SheetActivity[]; materialOver: MaterialItem[];
  fullAccess: boolean; role: string; prefs: Prefs;
};
...
const STATUS_COLOR: Record<string, string> = {
  tre: 'text-red-400', hoan_thanh: 'text-emerald-400', nghiem_thu: 'text-teal-400',
  dang_thi_cong: 'text-sky-400', chuan_bi: 'text-zinc-400',
};
const STATUS_LABEL: Record<string, string> = {
  tre: 'Đang trễ', hoan_thanh: 'Hoàn thành', nghiem_thu: 'Nghiệm thu',
  dang_thi_cong: 'Thi công', chuan_bi: 'Chuẩn bị',
};
...
const EVENT_LABEL: Record<EventType, string> = {
  progress: 'Cập nhật tiến độ', photo: 'Ảnh hiện trường',
  document: 'Bản vẽ / tài liệu', comment: 'Bình luận',
};
...
function FeedTaskCard({ item, variant }: { item: FeedTaskItem; variant: 'overdue' | 'due_soon' | 'upcoming' }) {
  const href = trackingHref(item.sheetSlug, item.code);
  const days = variant === 'overdue'
    ? daysOverdue(item.endDate!) : variant === 'due_soon'
    ? daysUntil(item.endDate!) : daysUntil(item.startDate!);
  const urgent = variant === 'overdue' ? days > 7 : variant === 'due_soon' ? days <= 2 : false;

  return (
    <a href={href}
      className={`block border rounded-xl p-4 transition group ${
        urgent ? 'bg-red-950/10 border-red-900/40' : 'bg-zinc-900/60 border-zinc-800 hover:border-zinc-600'
      }`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-1.5 flex-wrap">
            ...
            <span className="text-sm font-medium text-zinc-100 flex-1 leading-snug">{item.name}</span>
          ...
          <div className="mt-2.5"><ProgressBar value={item.progress} warn={variant === 'overdue'} /></div>
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            <span className={`text-xs font-medium ${
              variant === 'overdue' ? 'text-red-400' :
              variant === 'due_soon' ? (days <= 2 ? 'text-red-400' : 'text-amber-400') : 'text-sky-400'}`}>
              {variant === 'overdue' ? `Trễ ${days} ngày · hạn ${item.endDate}` :
               variant === 'due_soon' ? `Còn ${days} ngày · hạn ${item.endDate}` :
...
function PrefRow({ label, desc, prefKey, prefs, onToggle, saving }: {
  ...
  prefs: Prefs; onToggle: (key: PrefKey, val: boolean) => void; saving: boolean;
}) {
  ...
  return (
    <div className="flex items-center gap-4 py-3.5 border-b border-zinc-800/60 last:border-0">
      ...
      <button onClick={() => onToggle(prefKey, !enabled)} disabled={saving}
...
export default function MyTasksPage() {
  ...
  const loadFeed = useCallback(async (silent = false) => {
    if (!silent) setLoadingFeed(true); else setRefreshing(true);
    const r = await fetch('/api/notifications/feed').catch(() => null);
    if (r?.status === 401) { window.location.href = '/login'; return; }
    if (r?.ok) setFeed(await r.json());
    setLoadingFeed(false); setRefreshing(false);
  }, []);
  ...
  // Feed tab items
  type NavItem = { key: FeedTab; label: string; icon: React.ReactNode; count: number; activeCls: string };
  const feedTabs: NavItem[] = [
    { key: 'activity',  label: 'Hoạt động',     icon: <Activity className="w-4 h-4" />,     count: activityCount,            activeCls: 'bg-emerald-700' },
    { key: 'overdue',   label: 'Quá hạn',        icon: <AlertTriangle className="w-4 h-4" />, count: feed?.overdue.length ?? 0, activeCls: 'bg-red-700' },
    { key: 'due_soon',  label: 'Sắp hạn',        icon: <Clock className="w-4 h-4" />,         count: feed?.dueSoon.length ?? 0, activeCls: 'bg-amber-700' },
    { key: 'upcoming',  label: 'Sắp thi công',   icon: <CalendarClock className="w-4 h-4" />, count: feed?.upcomingStart.length ?? 0, activeCls: 'bg-sky-700' },
    ...(feed?.fullAccess ? [{ key: 'material' as FeedTab, label: 'Vật tư vượt ĐM', icon: <Package className="w-4 h-4" />, count: feed.materialOver.length, activeCls: 'bg-rose-700' }] : []),
    { key: 'settings',  label: 'Cài đặt',        icon: <Settings className="w-4 h-4" />,      count: 0,                        activeCls: 'bg-zinc-600' },
  ];
      ...
      <main className="max-w-3xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3.5">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-zinc-500 mb-1">Đang làm</p>
            <p className="text-3xl font-bold text-white">{(s?.total ?? 0) - (s?.done ?? 0)}</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3.5">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-red-500/80 mb-1 flex items-center gap-1">
              <AlertTriangle className="w-3 h-3" /> Đang trễ
            </p>
            <p className="text-3xl font-bold text-red-300">{s?.delayed ?? 0}</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3.5">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-500/80 mb-1 flex items-center gap-1">
        ...
        <div className="flex gap-1 p-1 bg-zinc-900 border border-zinc-800 rounded-xl">
          <button onClick={() => setSegment('tasks')}
          ...
          <button onClick={() => setSegment('notifications')}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-semibold transition ${
              segment === 'notifications' ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-zinc-200'
            }`}>
          ...
          <>
            ...
            {bySheet.size === 0 ? (
              <div className="py-16 text-center text-zinc-500 bg-zinc-900 border border-zinc-800 rounded-xl">
                ...
                <p className="text-xs text-zinc-600 mt-1">Liên hệ PM để được phân công.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {[...bySheet.entries()].map(([sheetType, g]) => {
                  ...
                  return (
                    <div key={sheetType} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                      {/* Sheet header */}
                      <a href={g.slug ? `/tracking/${g.slug}` : '#'}
                        className="flex items-center gap-4 px-4 py-3.5 hover:bg-zinc-800/60 transition group border-b border-zinc-800">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs font-bold uppercase tracking-wide text-zinc-400">{sheetType}</span>
                            {g.delayed > 0 && (
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex-1 bg-zinc-800 rounded-full h-2 overflow-hidden">
                              <div className={`h-2 rounded-full transition-all ${pct >= 80 ? 'bg-emerald-500' : pct >= 50 ? 'bg-sky-500' : 'bg-amber-500'}`}
                                style={{ width: `${pct}%` }} />
                            </div>
                            <span className="text-sm font-bold tabular-nums text-zinc-100 shrink-0">{pct}%</span>
                          </div>
                          <p className="text-xs text-zinc-600 mt-1">{g.done}/{g.total} hoàn thành</p>
                        </div>
                              ...
                              <a href={slug ? `/tracking/${slug}?search=${encodeURIComponent(t.code)}` : '#'} className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-3 mt-1.5">
                                  <div className="w-20 bg-zinc-800 rounded-full h-1.5 overflow-hidden shrink-0">
                                    <div className="h-1.5 rounded-full bg-emerald-500" style={{ width: `${pctTask}%` }} />
                                  </div>
                                  <span className="text-xs text-zinc-500 tabular-nums">{pctTask}%</span>
                                  {t.endDate && (
        ...
        {segment === 'notifications' && (
          <>
            ...
            <div className="flex items-center justify-between gap-2">
                  ...
                  return (
                    <button key={tab.key} onClick={() => setFeedTab(tab.key)}
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition shrink-0 ${
                        isActive ? `${tab.activeCls} text-white` : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/60'
                      }`}>
                      {tab.icon}
              ...
              <button onClick={() => loadFeed(true)} disabled={refreshing || loadingFeed}
                className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-200 border border-zinc-800 hover:border-zinc-600 rounded-lg px-2.5 py-2 transition shrink-0 disabled:opacity-40">
            ...
            {loadingFeed ? (
              ...
              <Empty icon={<Bell className="w-10 h-10" />} label="Không tải được thông báo. Kéo làm mới để thử lại." />
            ) : (
              <div className="space-y-3">
                {feedTab === 'activity' && (
                  <>
                    <p className="text-xs text-zinc-600">{activityCount > 0 ? `${activityCount} hoạt động${feed.fullAccess ? ' — toàn dự án' : ''}` : 'Chưa có hoạt động nào'}</p>
                    {feed.recentActivity.length === 0
                      ? <Empty icon={<Activity className="w-10 h-10" />} label="Chưa có hoạt động nào gần đây" />
                ...
                {feedTab === 'overdue' && (
                  <>
                    <p className="text-xs text-zinc-600">{feed.overdue.length} công việc quá hạn chưa hoàn thành</p>
                    {feed.overdue.length === 0
                      ? <Empty icon={<Check className="w-10 h-10" />} label="Tốt! Không có công việc nào quá hạn" />
                      : feed.overdue.map(t => <FeedTaskCard key={t.id} item={t} variant="overdue" />)}
                  </>
                )}

                {feedTab === 'due_soon' && (
                  <>
                    <p className="text-xs text-zinc-600">{feed.dueSoon.length} công việc đến hạn trong 5 ngày tới</p>
                    {feed.dueSoon.length === 0
                      ? <Empty icon={<Check className="w-10 h-10" />} label="Không có công việc nào sắp đến hạn" />
                      : feed.dueSoon.map(t => <FeedTaskCard key={t.id} item={t} variant="due_soon" />)}
                  </>
                )}

                {feedTab === 'upcoming' && (
                  <>
                    <p className="text-xs text-zinc-600">{feed.upcomingStart.length} công việc bắt đầu trong 7 ngày tới</p>
                    {feed.upcomingStart.length === 0
                      ? <Empty icon={<CalendarClock className="w-10 h-10" />} label="Không có công việc nào sắp bắt đầu" />
                      : feed.upcomingStart.map(t => <FeedTaskCard key={t.id} item={t} variant="upcoming" />)}
                  </>
                )}

                {feedTab === 'material' && feed.fullAccess && (
                  <>
                    <p className="text-xs text-zinc-600">{feed.materialOver.length} vật tư đang vượt định mức</p>
                            ...
                            <a key={m.id} href="/materials"
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-zinc-500 mt-0.5">
                                  ...
                                  Dùng {m.qtyUsed}/{m.qtyPlanned}{m.unit ? ` ${m.unit}` : ''} · vượt {m.qtyUsed - m.qtyPlanned}{m.unit ? ` ${m.unit}` : ''}
                                </p>
                              </div>
                ...
                {feedTab === 'settings' && (
                  <div className="space-y-4">
                    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
                      ...
                      <p className="text-xs text-zinc-500 mb-3">Được xác định theo vai trò, không thể thay đổi.</p>
                      <div className={`rounded-lg px-3 py-2.5 text-sm ${feed.fullAccess ? 'bg-emerald-950 text-emerald-300' : 'bg-zinc-800 text-zinc-300'}`}>
                        {feed.fullAccess ? '✓ Toàn bộ dự án — Admin, PM, Kỹ sư' : '⚙ Chỉ công việc được giao — Thầu phụ'}
                      </div>
                    </div>
                    <div className="bg-zinc-900 border border-zinc-800 rounded-xl px-4">
                      ...
                      <p className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wide pb-2">Nhắc nhở tiến độ</p>
                      <PrefRow prefKey="delayed"           label="Công việc quá hạn"      desc="Thông báo khi task qua ngày kết thúc mà chưa hoàn thành"       prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'delayed'} />
                      <PrefRow prefKey="due_soon"          label="Sắp đến hạn (5 ngày)"   desc="Cảnh báo sớm task đến hạn trong 5 ngày tới"                    prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'due_soon'} />
                      <PrefRow prefKey="upcoming_start"    label="Sắp thi công (7 ngày)"  desc="Công việc bắt đầu trong 7 ngày tới để chuẩn bị nhân lực, vật tư" prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'upcoming_start'} />
                      <p className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wide pb-2 pt-4">Hoạt động</p>
                      <PrefRow prefKey="activity_progress" label="Cập nhật tiến độ"        desc="Ai đó tick checkbox hoặc sửa % tiến độ"                        prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'activity_progress'} />
                      <PrefRow prefKey="activity_photo"    label="Ảnh hiện trường"         desc="Ảnh công trường mới được tải lên"                              prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'activity_photo'} />
                      <PrefRow prefKey="activity_document" label="Bản vẽ / tài liệu"       desc="Biên bản nghiệm thu hoặc file tài liệu mới"                    prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'activity_document'} />
                      <PrefRow prefKey="activity_comment"  label="Bình luận"               desc="Bình luận mới trên công việc"                                  prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'activity_comment'} />
                      {feed.fullAccess && <>
                        <p className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wide pb-2 pt-4">Vật tư</p>
                        <PrefRow prefKey="material_over" label="Vật tư vượt định mức" desc="Vật tư sử dụng vượt mức kế hoạch" prefs={feed.prefs} onToggle={togglePref} saving={savingPref === 'material_over'} />
                      </>}
                      <div className="py-3 text-xs text-zinc-600">{savingPref ? 'Đang lưu...' : 'Thay đổi được lưu tự động.'}</div>
                    </div>
                  </div>
                )}
              </div>
