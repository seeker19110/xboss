  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      <AppHeader
        ...
        subtitle="Tạm ứng, thanh toán, cấp phát vật tư và đề xuất kỹ thuật trực tuyến"
        bottomActions={
          canCreate ? (
            <button
      ...
      <main className="p-4 sm:p-6 pb-24 space-y-6 max-w-screen-2xl mx-auto">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bento-card p-4 flex flex-col justify-between">
            ...
            <p className="text-2xl font-bold font-mono text-sky-400 mt-2">{submitted.length}</p>
          </div>
          <div className="bento-card p-4 flex flex-col justify-between">
            <span className="text-xs font-semibold uppercase tracking-wide text-zinc-400">
              Được duyệt
          ...
          <div className="bento-card p-4 flex flex-col justify-between">
            ...
            <p className="text-2xl font-bold font-mono text-zinc-300 mt-2">
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition ${
                tab === t.key
                  ? t.key === "inbox"
                    ? "bg-sky-500/10 border border-sky-500/40 text-sky-300 shadow-xs"
                    : "bg-zinc-800 text-white shadow-xs"
                  : "text-zinc-400 hover:text-zinc-200"
              }`}
            >
              {t.key === "inbox" && <Inbox className="w-3.5 h-3.5" />}
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {canSubmit && (
            <button
              onClick={submit}
              disabled={busy}
              className="flex items-center gap-2 bg-sky-700 hover:bg-sky-800 disabled:opacity-50 px-4 py-2 rounded-lg text-sm font-semibold transition text-on-accent"
            >
          ...
          {canDecide && (
            <>
              <button
                onClick={approve}
                disabled={busy}
                className="flex items-center gap-2 bg-emerald-700 hover:bg-emerald-800 disabled:opacity-50 px-4 py-2 rounded-lg text-sm font-semibold transition text-on-accent"
              >
