...
export default function PaymentCertsRedirectPage() {
  const router = useRouter();

  useEffect(() => {
    router.replace("/commercial?tab=ipc-payments&sub=ipc");
  }, [router]);
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        <div className="space-y-1">
          <h2 className="text-lg font-bold text-zinc-100">
            Đang chuyển tiếp đến Chứng Chỉ Thanh Toán IPC...
          </h2>
          <p className="text-xs text-zinc-400">
            Nghiệm thu khối lượng và chứng chỉ IPC đã được hợp nhất tại{" "}
            <b>/commercial?tab=ipc-payments&sub=ipc</b>.
          </p>
        </div>
        <a
