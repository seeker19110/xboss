...
- **Bảo mật:** **API là ranh giới bảo mật duy nhất** — mọi route gọi `getCurrentUser()` + kiểm quyền `CAN`/`canTouchTask`. Phiên stateless HMAC; rate-limit login; SQL tham số hoá qua `lib/db`. **RLS Postgres là phòng tuyến thứ 2** (không thay tầng app): bật trên **11 bảng tài chính** (`0069` mở, `0077` khoá cửa), nhóm bảng theo tổ chức, **3 bảng Zalo Field Copilot** (`0119`), **`cad_takeoff_boq_map`** (`0140` — map mã BOQ theo dự án của plugin AutoCAD), **`cad_schematic_graphs`** (`0146` — đồ thị sơ đồ nguyên lý theo dự án), và **toàn bộ 15 bảng `engineering_*`** (`0092` — vào thẳng policy nghiêm ngặt, không có nhánh "thiếu ngữ cảnh → cho qua"). Chạy bằng role `xboss_app` NOBYPASSRLS — xem ADR-0005.
...
- **Đã ra mắt nội bộ** (v0.3.0) — đã hoàn tất **M0–M52** (tracking lõi + toàn bộ mở rộng chuỗi giá trị xây dựng + đợt "lên tầm ERP"), thêm M56/M58/M59/M61–M64 và track **ENG-1→ENG-5** (Engineering OS nền tảng, chưa có traffic thật từ MEPF-Agents). Đang tinh chỉnh chất lượng/UX liên tục (xem `git log`, `PROGRESS.md`).
