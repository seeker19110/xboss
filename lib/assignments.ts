...
export async function assignSheetManager(
  ...
  const prev = await queryOne<{ manager_id: number | null; code: string; slug: string | null }>(
    `SELECT manager_id, code, slug FROM sheet_types WHERE id = ?`,
    sheetId,
  );
  ...
  await logAssignment(
    "sheet",
    sheetId,
    prev?.code ?? `sheet#${sheetId}`,
    prev?.manager_id ?? null,
    userId,
    changedBy,
    userId !== null,
  );
  ...
  await run(
    `UPDATE work_packages SET assigned_to = ?
      WHERE sheet_type_id = ? AND assigned_manual = FALSE`,
    userId,
    sheetId,
  );
  await run(
    `UPDATE tasks SET assigned_to = ?, updated_at = CURRENT_TIMESTAMP
     ...
     WHERE tasks.package_id = wp.id AND wp.sheet_type_id = ?
       AND wp.assigned_manual = FALSE AND tasks.assigned_manual = FALSE`,
    userId,
    sheetId,
  );
...
export async function assignPackage(
  ...
  const prev = await queryOne<{
    ...
    `SELECT wp.assigned_to, wp.code, wp.name, st.slug AS sheet_slug
       FROM work_packages wp JOIN sheet_types st ON wp.sheet_type_id = st.id WHERE wp.id = ?`,
    packageId,
  );
  ...
  if (userId === null) {
    const r = await queryOne<{ manager_id: number | null }>(
      `SELECT st.manager_id FROM work_packages wp
        JOIN sheet_types st ON wp.sheet_type_id = st.id WHERE wp.id = ?`,
      packageId,
    );
  ...
  await run(
    `UPDATE tasks SET assigned_to = ?, updated_at = CURRENT_TIMESTAMP
      WHERE package_id = ? AND assigned_manual = FALSE`,
    effective,
    packageId,
  );
  ...
  if (effective) {
    const taskCount = await queryOne<{ n: number }>(
      `SELECT COUNT(*) AS n FROM tasks WHERE package_id = ? AND assigned_manual = FALSE`,
      packageId,
    );
  ...
  if (userId === null) {
    const r = await queryOne<{ assigned_to: number | null }>(
      `SELECT wp.assigned_to FROM tasks t
        JOIN work_packages wp ON t.package_id = wp.id WHERE t.id = ?`,
      taskId,
    );
...
export async function userWorkloads(): Promise<Map<number, { total: number; delayed: number }>> {
  const rows = await query<{ userId: number; total: number; delayed: number }>(
    `SELECT t.assigned_to AS "userId",
            COUNT(*) AS total,
            COUNT(*) FILTER (WHERE COALESCE(t.end_date, wp.end_date) IS NOT NULL AND COALESCE(t.end_date, wp.end_date) < ?
  ...
  for (const r of rows)
    m.set(Number(r.userId), { total: Number(r.total), delayed: Number(r.delayed) });
  return m;
}
