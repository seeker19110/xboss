// lib/dich-vu/thong-bao.ts — Dịch vụ đồng bộ + liệt kê thông báo (tầng dịch vụ, ADR-0008).
//
// Trước đây toàn bộ ~1.080 dòng này nằm trong app/api/notifications/route.ts. Nó phối hợp
// hơn 20 miền (tiến độ, tài chính, hiện trường, kỹ thuật, vật tư…) nên không thuộc về miền
// nào cả — đặt vào bất kỳ miền nào cũng khiến miền đó phải biết về 19 miền còn lại.
//
...
import { pendingVariations } from "@/lib/tai-chinh/vo";
import { overContractCerts, pendingCerts } from "@/lib/tai-chinh/paymentcerts";
import { dueCorrespondences } from "@/lib/hien-truong/correspondence";
import { stageMissingList } from "@/lib/tien-do/constructionStages";
import { calibrationDueList } from "@/lib/vat-tu/equipment";
import { overNormItems, NORM_OVER_THRESHOLD_PCT } from "@/lib/khoi-luong/norms";
import { openHseActions } from "@/lib/hien-truong/hse";
import { overdueMeetingActions } from "@/lib/hien-truong/meetings";
import { formatDateTimeVN } from "@/lib/nen/date";
import { pendingProposalsOver } from "@/lib/tai-chinh/proposals";
import { expiringLegalDocs } from "@/lib/hien-truong/kickoff";
import { expiringCertifications } from "@/lib/hien-truong/hr";
import { expiringInsuranceBonds } from "@/lib/tai-chinh/insurance";
import { expiringEnvPermits, exceededMonitoring } from "@/lib/hien-truong/environment";
import { alarmingPoints } from "@/lib/hien-truong/monitoring";
...
import { PROPOSAL_PENDING_DAYS } from "@/lib/tai-chinh/proposals";
...
export async function syncAndListNotifications(
  ...
  return { notifications: items, unread: unread?.count ?? 0 };
}
