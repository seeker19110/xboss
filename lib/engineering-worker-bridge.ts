        ...
        const createdObj = (await createEngineeringObject(
          {
            ...
            externalKey: item.external_key || `WORKER-${task.id.slice(0, 6)}-${idx + 1}`,
            name: objName,
            properties: {
        ...
        const createdObj = (await createEngineeringObject(
          {
            ...
            externalKey: clash.clash_id || `CLASH-${task.id.slice(0, 6)}-${idx + 1}`,
            name:
              clash.name ||
      ...
      const createdObj = (await createEngineeringObject(
        {
          ...
          discipline: "MEPF",
          externalKey: `TASK-RES-${task.id.slice(0, 8)}`,
        ...
        suggestions: [
          {
            ...
            externalObjectKey: createdObjectIds[0],
            evidence: [
              {
                kind: "fact",
